...
export function ProductsPanel() {
        ...
        <form id="product-form" onSubmit={handleSubmit} className="flex-1 overflow-auto">
          ...
          <div className="p-6 lg:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-5">
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">SKU *</label><input name="sku" defaultValue={editId ? undefined : ''} disabled={!!editId} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text disabled:text-theme-text-muted/50 focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Código de barra</label><input name="barcode" defaultValue={form.barcode} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Código interno</label><input name="internal_code" defaultValue={form.internal_code} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Descripción corta</label><input name="short_description" defaultValue={form.short_description} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 space-y-1"><label className="text-xs text-theme-text-muted/70">Descripción *</label><input name="description" defaultValue={form.description} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                ...
                <ClassifierCombobox type="SALES_UNIT" label="Unidad venta" value={form.sales_unit} onChange={v => setForm(p => ({ ...p, sales_unit: v }))} />
                 ...
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Stock máximo</label><input name="max_stock" type="number" step="0.001" defaultValue={form.max_stock} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Punto reposición</label><input name="reorder_point" type="number" step="0.001" defaultValue={form.reorder_point} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">IVA %</label><input name="tax_rate" type="number" step="0.01" defaultValue={form.tax_rate} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40" /></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Perecible</label><select name="is_perishable" defaultValue={form.is_perishable} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40"><option value="false" className="bg-theme-surface text-theme-text">NO</option><option value="true" className="bg-theme-surface text-theme-text">SI</option></select></div>
                 ...
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Requiere vencimiento</label><select name="requires_expiration" defaultValue={form.requires_expiration} className="w-full h-9 rounded-lg border border-theme-border bg-theme-surface px-3 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40"><option value="false" className="bg-theme-surface text-theme-text">NO</option><option value="true" className="bg-theme-surface text-theme-text">SI</option></select></div>
                 <div className="space-y-1"><label className="text-xs text-theme-text-muted/70">Imagen</label><input name="image" type="file" accept="image/jpeg,image/png,image/webp" ref={imgRef} className="w-full text-xs text-theme-text-muted/70 file:mr-3 file:rounded-lg file:border file:border-theme-border file:bg-theme-text/5 file:px-3 file:py-1.5 file:text-xs file:text-theme-text-accent file:cursor-pointer" />{form.existing_image && <p className="text-[10px] text-theme-text-muted/50 mt-1">Imagen actual subida</p>}<input name="existing_image" type="hidden" value={form.existing_image} /></div>
                 <div className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 space-y-1"><label className="text-xs text-theme-text-muted/70">Observaciones</label><textarea name="notes" defaultValue={form.notes} rows={2} className="w-full rounded-lg border border-theme-border bg-theme-surface px-3 py-2 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40 resize-none" /></div>
            </div>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-theme-surface">
      ...
      {preview && (
         <div className="rounded-2xl border border-theme-border bg-theme-text/[0.025] p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-theme-text">Vista previa - {preview.rows.length} filas válidas</h3>
            <div className="flex gap-2">
               <button onClick={() => setPreview(null)} disabled={isImporting} className="px-3 py-1.5 rounded-lg border border-theme-border text-xs text-theme-text-muted/70 hover:text-theme-text disabled:opacity-40 disabled:cursor-not-allowed">Cancelar</button>
      ...
      {loading ? (
         <div className="rounded-2xl border border-theme-border bg-theme-text/[0.025] p-10 text-center"><p className="text-theme-text-muted/50 text-sm">Cargando...</p></div>
      ) : products.length === 0 && viewMode === 'table' ? (
         <div className="rounded-2xl border border-theme-border bg-theme-text/[0.025] p-10 text-center"><p className="text-theme-text-muted/50 text-sm">No hay productos en el catálogo.</p></div>
      ) : viewMode === 'cards' ? (
        <div className="flex-1 overflow-auto p-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {(products.length > 0 ? products : filteredData).map(p => (
              <div key={p.id} onClick={() => toggleSelect(p.id)} className={`rounded-2xl border overflow-hidden bg-theme-surface hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group cursor-pointer ${selected.has(p.id) ? 'border-emerald-500/60 ring-1 ring-emerald-500/30' : 'border-theme-border'}`}>
                <div className={`aspect-[4/3] flex items-center justify-center overflow-hidden ${selected.has(p.id) ? 'bg-theme-accent/10' : 'bg-theme-text/5'}`}>
                ...
                <div className="p-4 space-y-2 bg-theme-text/[0.025]">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {p.brand && <span className="text-[10px] px-2 py-0.5 rounded-full bg-theme-text/5 text-theme-text-muted/70">{p.brand}</span>}
                  </div>
      ...
      {totalPages > 1 && viewMode === 'table' && (
        <div className="shrink-0 flex flex-col gap-3 border-t border-theme-border/60 bg-theme-text/[0.012] px-5 py-3 text-xs sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <select value={filters.pageSize} onChange={e => setFilter('pageSize', e.target.value)} className="h-8 rounded-lg border border-theme-border bg-theme-surface px-2 text-xs text-theme-text focus:outline-none focus:ring-1 focus:ring-theme-border-accent/40">
