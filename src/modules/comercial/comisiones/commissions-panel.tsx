...
function sellerDraft(seller: CommissionSeller): SellerDraft {
  return {
...
export function CommissionsPanel({ isSuperUser }: { isSuperUser: boolean }) {
  ...
  const [annulledList, setAnnulledList] = useState<
  ...
  const loadSellers = async () => {
    ...
    try {
      ...
      setDrafts(
        Object.fromEntries(
          rows.map((row) => [row.seller_bsale_id, sellerDraft(row)]),
        ),
      );
    } catch (err) {
      ...
      {view === "main" ? (
        ...
        <Configuration
          ...
          onSettingsChange={(percent) =>
            setSettings((current) =>
              current
                ? { ...current, default_commission_percent: percent }
                : current,
            )
          }
...
function SimulateTab({
  ...
  const filtered = useMemo(() => {
    ...
    return preview.lines.filter((l) => {
      if (
        filters.invoice &&
        !String(l.original_invoice_id || l.invoice_number || "").includes(
          filters.invoice,
        )
      )
        return false;
...
function AnnulledTab({
  ...
  annulled: CommissionSettlementHeader[];
  busy: boolean;
  detail: SettlementDetail | null;
  onLoad: () => void;
  onDetail: (id: string) => void;
...
function SettlementDetailView({
  ...
  const ncLinesCount = lines.filter(
    (l) => l.line_type === "CREDIT_NOTE",
  ).length;
    ...
    <div>
      <div className="sticky top-0 z-10 -mx-4 border-b border-theme-border bg-theme-surface/95 px-4 pb-2 pt-0 backdrop-blur-sm md:-mx-6 md:px-6">
        ...
        <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-0.5 text-[11px]">
          <span>
            <b>Facturas:</b> {invoicesCount}
          </span>
          <span>
            <b>Líneas:</b> {linesCount}
          </span>
          <span>
            <b>Líneas NC:</b> {ncLinesCount}
          </span>
          <span>
            <b>Neto +:</b> {money(netoPositivo)}
          </span>
          <span className="text-amber-700">
            <b>Neto NC:</b> {money(netoNc)}
          </span>
          <span>
            <b>Neto final:</b> {money(netoFinal)}
          </span>
          <span>
            <b>Comisión:</b> {money(comisionFinal)}
          </span>
          <span>
            <b>% efectivo:</b>{" "}
            {netoFinal
              ? `${((comisionFinal / netoFinal) * 100).toFixed(2)}%`
              : "0.00%"}
          </span>
          <span className="text-theme-text-muted">
            Creado {fmt(header.created_at || null)}
          </span>
        </div>
      </div>
...
function PreviewReport({
  ...
  return (
    <section className="space-y-2.5">
      ...
      {general > 0 && (
        <div
          className={`rounded-lg border px-3 py-1.5 text-[11px] ${ncCount > 0 ? "border-amber-500/30 bg-amber-500/8 text-amber-700" : "border-amber-500/30 bg-amber-500/8 text-amber-700"}`}
        >
          <span className="font-semibold">
            {general}{" "}
            {general === 1 ? "línea de venta usa" : "líneas de venta usan"} la
            comisión general.
          </span>
          {ncCount > 0 ? (
            <>
              {" "}
              {ncCount}{" "}
              {ncCount === 1 ? "línea corresponde" : "líneas corresponden"} a
              notas de crédito aplicadas como descuento.
            </>
          ) : (
            " Configura reglas específicas para personalizar."
          )}
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <td>
                  <span
                    className={
                      isNC ? "text-amber-700 font-medium" : "font-medium"
                    }
                  >
