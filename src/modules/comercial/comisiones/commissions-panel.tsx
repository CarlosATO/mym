...
export function CommissionsPanel() {
  ...
  return <div className="commission-panel flex h-full min-h-0 flex-col overflow-hidden bg-theme-surface text-theme-text">
    ...
    </main> : <Configuration tab={tab} setTab={setTab} sellers={sellers} drafts={drafts} settings={settings} groups={groups} rules={rules} busy={busy} onSellerChange={(id, changes) => setDrafts(current => ({ ...current, [id]: { ...current[id], ...changes } }))} onSaveSeller={saveSeller} onSettingsChange={percent => setSettings(current => current ? { ...current, default_commission_percent: percent } : current)} onSaveSettings={async (percent?: number) => { const pct = percent ?? settings?.default_commission_percent; if (pct == null) return; setBusy(true); try { setSettings(await updateCommissionSettings({ default_commission_percent: pct })) } catch (err) { setError(errorMessage(err)) } finally { setBusy(false) } }} onRefresh={loadConfig} setError={setError} />}
  </div>
}
...
function General({ settings, busy, onSettingsChange, onSaveSettings }: { settings: CommissionSettings | null; busy: boolean; onSettingsChange: (percent: number) => void; onSaveSettings: (percent?: number) => void }) {
