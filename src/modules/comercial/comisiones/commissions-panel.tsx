...
export function CommissionsPanel() {
  ...
  return <div className="commission-panel flex h-full min-h-0 flex-col overflow-hidden bg-theme-surface text-theme-text">
    ...
    {view === 'main' ? <main className="flex-1 overflow-auto px-4 py-4 md:px-6">
...
function CommissionStyles() { return <style jsx global>{`.commission-panel .btn-primary,.commission-panel .btn-secondary{display:inline-flex;align-items:center;justify-content:center;gap:.375rem;min-height:2rem;border-radius:.5rem;padding:.35rem .7rem;font-size:.75rem;font-weight:700}.commission-panel .btn-primary{background:var(--theme-accent);color:#fff}.commission-panel .btn-secondary{border:1px solid var(--theme-border);background:var(--theme-surface);color:var(--theme-text)}.commission-panel input,.commission-panel select,.commission-panel textarea{width:100%;border:1px solid var(--theme-border);border-radius:.5rem;background:var(--theme-surface);color:var(--theme-text);padding:.45rem .6rem;font-size:.75rem}.commission-panel table,.commission-panel th,.commission-panel td{color:var(--theme-text)!important}.commission-panel th{background:var(--theme-bg);padding:.55rem .6rem;text-align:left}.commission-panel td{padding:.55rem .6rem;border-top:1px solid var(--theme-border)}.commission-panel tbody tr:hover{background:var(--theme-surface-hover)}`}</style> }
...
function SimulateTab({ sellerId, setSellerId, periodFrom, setPeriodFrom, periodTo, setPeriodTo, busy, sellers, preview, onSimulate, onCreateDraft }: {
  ...
  return <div className="w-full space-y-4">
    <section className="rounded-xl border border-theme-border bg-theme-bg/35 p-4">
      <div className="grid gap-3 md:grid-cols-[1fr_150px_150px_auto_auto]">
        <Field label="Vendedor"><select value={sellerId} onChange={event => { setSellerId(event.target.value); if (event.target.value) onSimulate(event.target.value) }}><option value="">Selecciona un vendedor</option>{sellers.map(seller => <option key={seller.seller_bsale_id} value={seller.seller_bsale_id}>{seller.seller_name}</option>)}</select></Field>
        ...
        <button disabled={!sellerId || busy} onClick={() => onSimulate()} className="btn-primary self-end">{busy ? 'Simulando...' : 'Simular'}</button>
        {preview && preview.lines.length > 0 && <button disabled={busy} onClick={onCreateDraft} className="btn-primary self-end bg-emerald-600 hover:bg-emerald-700"><FileUp className="h-3.5 w-3.5" />Crear borrador</button>}
      </div>
    </section>
  ...
  return <div>
    ...
    <div className="mt-3 overflow-auto rounded-xl border border-theme-border"><table className="min-w-[1200px] w-full text-xs"><thead><tr><th>Tipo</th><th>Factura</th><th>SKU / Producto</th><th className="text-right">Cant.</th><th className="text-right">Neto</th><th className="text-right">%</th><th className="text-right">Comisión</th></tr></thead><tbody>{lines.map(line => {
      ...
      return <tr key={line.id} className={isNC ? 'opacity-85' : ''}>
        ...
        <td className="text-right">{line.commission_percent != null ? `${line.commission_percent}%` : '-'}</td>
...
function PreviewReport({ preview }: { preview: CommissionPreview }) {
  ...
  return <section className="space-y-3"><div className="rounded-xl border border-theme-border bg-theme-bg/35 px-3 py-2"><div className="grid gap-x-5 gap-y-1 sm:grid-cols-2 lg:grid-cols-5"><Kpi label="Facturas" value={invoices.toLocaleString('es-CL')} /><Kpi label="Líneas" value={filtered.length.toLocaleString('es-CL')} /><Kpi label="Neto" value={money(net)} /><Kpi label="Comisión estimada" value={money(commission)} /><Kpi label="% efectivo" value={`${net ? (commission / net * 100).toFixed(2) : '0.00'}%`} /></div></div><div className="rounded-xl border border-theme-border bg-theme-surface p-3"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-theme-text"><SlidersHorizontal className="h-3.5 w-3.5 text-theme-accent" />Filtros de simulación</div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[180px_1fr_1fr_1fr_140px_auto]"><input value={filters.invoice} onChange={e => setFilters(current => ({ ...current, invoice: e.target.value }))} placeholder="Factura" /><input value={filters.supplier} onChange={e => setFilters(current => ({ ...current, supplier: e.target.value }))} placeholder="Proveedor o grupo" /><input value={filters.product} onChange={e => setFilters(current => ({ ...current, product: e.target.value }))} placeholder="SKU o producto" /><select value={filters.rule} onChange={e => setFilters(current => ({ ...current, rule: e.target.value }))}><option value="">Todas las reglas</option>{ruleLabels.map(label => <option key={label} value={label}>{label}</option>)}</select><select value={filters.percent} onChange={e => setFilters(current => ({ ...current, percent: e.target.value }))}><option value="">Todos los %</option>{percentages.map(percent => <option key={percent} value={percent}>{percent}%</option>)}</select><button onClick={() => setFilters({ invoice: '', supplier: '', product: '', rule: '', percent: '' })} className="btn-secondary"><X className="h-3.5 w-3.5" />Limpiar</button></div></div>{general > 0 && ncCount > 0 ? <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700"><span className="font-semibold">{general} {general === 1 ? 'línea de venta usa' : 'líneas de venta usan'} la comisión general.</span> {ncCount} {ncCount === 1 ? 'línea corresponde' : 'líneas corresponden'} a notas de crédito aplicadas como descuento.</div> : general > 0 ? <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700"><span className="font-semibold">{general} {general === 1 ? 'línea usa' : 'líneas usan'} la comisión general.</span> Configura reglas específicas para personalizar.</div> : null}<PreviewTable lines={filtered} /></section>
}
...
function PreviewTable({ lines }: { lines: CommissionPreviewLine[] }) {
  ...
  return <div className="overflow-auto rounded-xl border border-theme-border">
    {ncCount > 0 && <div className="flex items-center gap-2 border-b border-theme-border bg-amber-500/10 px-3 py-2 text-xs text-amber-700"><span className="font-semibold">{ncCount} {ncCount === 1 ? 'línea es' : 'líneas son'} de nota de crédito.</span><span>Las NC descuentan línea por línea (matching por SKU).</span></div>}
    <table className="min-w-[1320px] w-full text-xs"><thead><tr><th>Factura</th><th>Cliente</th><th>Pago</th><th>SKU / Producto</th><th>Proveedor / Grupo</th><th className="text-right">Cant.</th><th className="text-right">Neto</th><th>Regla</th><th className="text-right">%</th><th className="text-right">Comisión</th><th>Origen</th></tr></thead><tbody>{lines.map(line => {
      const isNC = line.commission_line_type === 'CREDIT_NOTE_LINE'
      return <tr key={line.invoice_line_id} className={isNC ? 'opacity-85' : ''}>
        <td>{isNC ? `${line.original_invoice_number || line.invoice_number} (NC ${line.source_document_number || ''})` : line.invoice_number || line.invoice_bsale_id}</td>
        <td className="max-w-[160px] truncate">{line.customer_name}</td>
        <td>{line.payment_completed_at?.slice(0, 10)}</td>
        <td><b>{line.sku}</b><div>{line.product_name}{isNC && line.adjustment_reason ? <div className="mt-0.5 text-[10px] text-amber-600">{line.adjustment_reason}</div> : null}</div></td>
        <td><div>{line.supplier_name}</div><div>{line.commission_group_name || 'Sin grupo'}</div></td>
        <td className="text-right">{isNC ? `(${Math.abs(line.quantity)})` : line.quantity}</td>
        <td className={`text-right ${isNC ? 'text-amber-700' : ''}`}>{isNC ? `(${money(Math.abs(line.net_amount))})` : money(line.net_amount)}</td>
        <td><b>{isNC ? 'Nota de crédito' : line.applied_rule_label}</b>{line.rule_id && !isNC && <div className="mt-0.5 text-[10px] text-theme-text-muted">{line.applied_rule_scope}</div>}</td>
        <td className="text-right">{line.commission_percent}%</td>
