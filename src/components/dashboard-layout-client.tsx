...
function UserMenu({ profile, activeCompany }: { profile: Profile & { roles: { name: string } }; activeCompany: Company | null }) {
...
export function DashboardLayoutClient({ children, profile, permissions, activeCompany }: DashboardLayoutClientProps) {
  ...
  const isModulePage = pathname.startsWith('/dashboard/adquisiciones')

  const allowedLinks = adminLinkDefs.filter(link => permissions.includes(link.permission))
          ...
          <div className="flex items-center gap-6 overflow-hidden">
            <Link href="/dashboard" className="flex items-center gap-3 shrink-0 group">
              ...
              <div className="hidden sm:block truncate leading-tight">
                <p className="font-bold text-sm text-theme-text max-w-[120px] truncate">
                  {activeCompany.trade_name || activeCompany.business_name}
                </p>
                <p className="text-[10px] font-semibold text-theme-accent uppercase tracking-[0.15em] truncate max-w-[120px]">
                  {activeCompany.rut || 'Empresa'}
                </p>
              </div>
            </Link>

            {allowedLinks.length > 0 && <div className="hidden md:block w-px h-8 bg-theme-border shrink-0" />}

            <nav className="hidden lg:flex items-center gap-1 overflow-x-auto hide-scrollbar">
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-theme-text/10 text-theme-text' : 'text-theme-text-muted/80 hover:bg-theme-text/5 hover:text-theme-text'
                    )}
                    title={link.label}
                  >
                    <Icon className="h-4 w-4" />
