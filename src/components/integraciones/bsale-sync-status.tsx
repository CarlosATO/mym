...
import { forceSyncBsaleProducts, forceSyncBsaleProductTypes, forceSyncBsaleSaleConditions, getSyncStatus } from '@/app/actions/integraciones/sync'
...
export function SaleConditionsBsaleSyncStatus() {
  const [status, setStatus] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  const [syncing, setSyncing] = useState(false)

  const loadStatus = async () => {
    const res = await getSyncStatus('BSALE', 'sale_conditions')
    setStatus(res)
    setLoading(false)
  }
  ...
  const handleForceSync = async () => {
    if (!confirm('¿Forzar sincronización de condiciones de venta desde Bsale?')) return
    setSyncing(true)
    try {
      const result = await forceSyncBsaleSaleConditions()
      if (result.status !== 'SUCCESS') throw new Error(result.message || 'Sync no finalizó correctamente')
      await loadStatus()
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="flex items-center gap-4 bg-theme-surface border border-theme-border rounded-xl px-3 h-11 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-bold uppercase tracking-wider text-theme-text-muted/70">Sync Cond. Venta</span>
          <span className="text-xs font-semibold text-theme-text">
            {isRunning ? 'En proceso' : status?.lastSuccess ? 'OK' : status?.lastRun && status.lastRun.status !== 'SUCCESS' ? 'Error' : 'Nunca'}
          </span>
        </div>
