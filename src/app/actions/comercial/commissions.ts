...
export type CommissionRuleType =
  "FIXED_PERCENT" | "RANGE_BY_AMOUNT" | "RANGE_BY_QUANTITY";
...
function mapSeller(row: CommissionSellerRow): CommissionSeller {
  return {
    ...
    seller_type: sellerTypes.includes(row.seller_type)
      ? row.seller_type
      : "OTHER",
    notes: null,
  };
...
export async function upsertCommissionSellerProfile(
  ...
  if (!sellerTypes.includes(input.seller_type))
    throw new Error("Tipo de vendedor inválido");
...
export async function upsertCommissionRule(
  ...
  if (!validTarget)
    throw new Error(
      "El destino de la regla no corresponde al ámbito seleccionado",
    );

  const minAmount =
    type === "RANGE_BY_AMOUNT" ? Number(input.min_amount) : null;
  const maxAmount =
...
export async function getCommissionRuleBatchDetail(ruleBatchId: string) {
  ...
  return {
    ...
    products: (products || []).map((product) => ({
      ...
      supplierName:
        productSupplierNames.get(product.id as string) ||
        "Sin proveedor asociado",
    })),
...
export async function previewCommissionSettlement(input: {
  ...
  const previewSupplierIds = Array.from(
    new Set(
      rawLines.map((row) => String(row.supplier_id || "")).filter(Boolean),
    ),
  );
  ...
  const previewSuppliersById = new Map(
    (previewSuppliers || []).map((supplier) => {
      const effective = supplier.parent_supplier_id
        ? previewParentsById.get(supplier.parent_supplier_id as string) ||
          supplier
        : supplier;
      return [
        supplier.id as string,
  ...
  const baseLines = rawLines.map((row) => ({
    ...row,
    supplier_id:
      previewSuppliersById.get(String(row.supplier_id || ""))?.id ||
      row.supplier_id,
    supplier_name:
      previewSuppliersById.get(String(row.supplier_id || ""))?.name ||
      row.supplier_name,
    seller_bsale_id: Number(row.seller_bsale_id),
  ...
  >[];
  const { data: supplierRules, error: supplierRulesError } =
    await commissionDb()
      .from("commission_rules")
      .select(
  ...
  const linesWithEffectiveSupplierRules = baseLines.map((line) => {
    ...
    const applicable = (supplierRules || []).find(
      (rule) =>
        ...
        (!rule.valid_to ||
          line.payment_completed_at.slice(0, 10) <= rule.valid_to),
    );
    return applicable
      ? {
          ...line,
...
export async function createCommissionSettlementDraft(input: {
  ...
  if (!result.success)
    throw new Error(result.error || "Error al crear borrador");
...
export async function issueCommissionSettlement(input: {
  settlement_id: string;
}) {
  const { companyId, userId } = await getAuthenticatedCompany();
...
export async function exportCommissionSettlementXlsx(
  ...
  const data = lines.map((line) => ({
    ...
    "Factura original": String(
      line.original_invoice_number || line.invoice_number || "",
    ),
    Cliente: line.customer_name || "",
    Pago: line.payment_completed_at || "",
    "Tipo línea":
      line.line_type === "CREDIT_NOTE" ? "Nota de crédito" : "Factura",
    Origen:
