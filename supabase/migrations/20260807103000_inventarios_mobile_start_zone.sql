-- Migración para endurecer el motor de tareas y crear el contrato móvil de inicio de zona.

-- ============================================================
-- 1. Endurecer start_inventory_task
-- ============================================================
...
DECLARE
    ...
    v_request_hash pg_catalog.text;
    v_task_session_id pg_catalog.uuid;
    v_task_session_zone_id pg_catalog.uuid;
...
BEGIN
    ...
    -- LOCK GLOBAL POR ACTOR (Garantiza que un usuario no pueda intentar iniciar tareas concurrentemente)
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtext('inventarios.actor_operational_task'),
        pg_catalog.hashtext(v_actor_id::text)
    );

    -- LOCK ESPECIFICO DE TAREA (Mantiene compatibilidad y orden: Global -> Específico)
    PERFORM pg_catalog.pg_advisory_xact_lock(
    ...
    -- =========================================================================
    -- NUEVO GUARD: PREVENIR MULTIPLES TAREAS IN_PROGRESS / PAUSED POR ACTOR
    -- =========================================================================
    IF EXISTS (
        SELECT 1
...
ALTER FUNCTION inventarios.start_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION inventarios.start_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION inventarios.start_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.start_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) TO service_role;


-- ============================================================
-- 2. Endurecer resume_inventory_task
-- ============================================================
...
BEGIN
    ...
    -- LOCK GLOBAL POR ACTOR
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtext('inventarios.actor_operational_task'),
        pg_catalog.hashtext(v_actor_id::text)
    );

    -- LOCK ESPECIFICO DE TAREA
    PERFORM pg_catalog.pg_advisory_xact_lock(
...
ALTER FUNCTION inventarios.resume_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION inventarios.resume_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION inventarios.resume_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.resume_inventory_task(pg_catalog.uuid, pg_catalog.uuid, integer, pg_catalog.uuid) TO service_role;


-- ============================================================
-- 3. Crear wrapper móvil start_my_counting_zone
-- ============================================================
...
BEGIN
    ...
    -- 2. Validar accesos y obtener metadatos de la tarea de la zona
    SELECT 
        z.company_id,
        t.id,
        t.version,
    ...
    -- Validar que la tarea esté en ASSIGNED para permitir iniciarla
    -- (Opcionalmente podríamos delegarlo en start_inventory_task, pero lo hacemos explícito según instrucciones)

    -- 3. Llamar al motor interno
...
ALTER FUNCTION inventarios.start_my_counting_zone(pg_catalog.uuid, pg_catalog.uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION inventarios.start_my_counting_zone(pg_catalog.uuid, pg_catalog.uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION inventarios.start_my_counting_zone(pg_catalog.uuid, pg_catalog.uuid) FROM anon;
GRANT EXECUTE ON FUNCTION inventarios.start_my_counting_zone(pg_catalog.uuid, pg_catalog.uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.start_my_counting_zone(pg_catalog.uuid, pg_catalog.uuid) TO service_role;
