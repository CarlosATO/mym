...
BEGIN
    ...
    IF v_is_counter THEN
        ...
        PERFORM 1 FROM inventarios.task_assignments ta WHERE ta.company_id = p_company_id AND ta.session_id = v_sid AND ta.task_id = v_tid AND ta.id = v_task_ca AND ta.user_id = v_actor_id AND ta.released_at IS NULL FOR SHARE;
