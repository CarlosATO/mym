-- =========================================================================================
-- MIGRATION: 20260811390050_inventarios_list_my_location_counts_name_fix.sql
-- =========================================================================================
-- Parte A: corregir la fuente del campo "name" en list_my_location_counts.
-- Antes: name = inventarios.snapshot_products.name (cargado con SKU o descripcion corta).
-- Ahora: name = concat_ws(' - ', bsale_products.name, bsale_variants.description),
--   igual identidad descriptiva que lookup_my_counting_product / resolve_my_scanned_barcode.
--   Fallback a snapshot_products.name si no hay variante Bsale o el concat queda vacio.
-- Cardinalidad 1:1 garantizada: (company_id, bsale_id) es UNIQUE en bsale_variants y
--   bsale_products (indices unicos verificados). No se une adquisiciones.products (evita
--   multiplicacion de filas). sku se mantiene de snapshot_products.sku.
-- Unico cambio funcional: fuente de "name". Firma, envelope, guardas, grants, OPEN/CLOSED,
--   filtro counted_by y root/current/CAS intactos.
