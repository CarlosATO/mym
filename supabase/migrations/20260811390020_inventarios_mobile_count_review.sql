-- =========================================================================================
-- MIGRATION M2: 20260811390020_inventarios_mobile_count_review.sql
-- =========================================================================================
-- Wrappers COUNTER de revision/correccion por ubicacion (Mejora B V1).
-- Replican la logica de mutacion de correct_inventory_count / invalidate_inventory_count
-- SIN exigir inventarios.counts.correct (no se otorga permiso global a COUNTER).
-- Mantienen count_entry_corrections, invalidated_*, idempotencia, locks y CAS.
-- Autorizacion contextual COUNTER: actor/empresa/session/task derivados en servidor.
-- Ubicacion OPEN o CLOSED de la tarea/zona del actor; nunca se reabre task_locations.
...
BEGIN
    ...
    -- Ubicacion pertenece a la zona/tarea
    ...
    -- Estado operativo de la ubicacion segun task_locations del actor (OPEN | CLOSED); no reabre.
    ...
    -- Conteos efectivos vigentes del actor en esa ubicacion, resolviendo root/current.
    WITH my_roots AS (
...
BEGIN
    ...
    -- Ubicacion pertenece a la zona/tarea (OPEN o CLOSED); no reabre.
    SELECT szl.id, szl.snapshot_location_id
    INTO v_szl_id, v_snapshot_location_id
    FROM inventarios.session_zone_locations szl
    WHERE szl.company_id = v_company_id
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_ACCESS_DENIED',
            DETAIL = pg_catalog.jsonb_build_object('message', 'No tienes acceso a esta ubicación.', 'retryable', false)::text;
    END IF;
    -- No exige OPEN: basta que la ubicacion pertenezca a la zona/tarea. No se toca task_locations.
    ...
    SELECT ce.session_id, ce.snapshot_id, ce.session_zone_id, ce.task_id, ce.task_cycle,
           ...
           ce.session_participant_id
    INTO v_sid, v_szid, v_rid, v_tid, v_cyc, v_pid, v_vid, v_lid, v_root_inv_at, v_participant_id
    FROM inventarios.count_entries ce
    ...
    INTO v_eff_inv_at, v_avail, v_dam, v_exp, v_blk, v_oth, v_eff_ident,
         v_eff_scanned, v_eff_recount, v_actor_id, v_participant_id
    FROM inventarios.count_entries ce
    ...
    -- Proteccion: solo correccion simple. Si el registro efectivo tiene distribucion en
    -- damaged/expired/blocked/other_unavailable, bloquear la correccion Mobile simple.
    ...
    SELECT ce.session_participant_id INTO v_participant_id
    FROM inventarios.count_entries ce WHERE ce.id = v_current_id;
...
BEGIN
    ...
    SELECT ce.session_id, ce.session_zone_id, ce.task_id, ce.task_cycle, ce.invalidated_at
    INTO v_sid, v_rid, v_tid, v_cyc, v_root_inv_at
    FROM inventarios.count_entries ce
    WHERE ce.company_id = v_company_id AND ce.id = p_root_count_entry_id FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_NOT_FOUND',
