-- Corrección forward del detalle de incidencias de códigos (V1):
-- el occurrence ahora expone evidence_id (id de evidence_files) cuando existe
-- evidencia vinculada (proposal_id directa o fallback por count_entry_id).
-- Esto habilita el flujo "Ver foto": la Server Action usa ese id para autorizar
-- la referencia y generar una signed URL corta, sin exponer paths permanentes.
--
-- Solo amplía el contrato de lectura existente; no modifica approve/reject ni
-- el modelo de alias.
--
