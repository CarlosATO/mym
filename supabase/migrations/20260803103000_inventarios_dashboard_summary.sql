-- Migration: 20260803103000_inventarios_dashboard_summary.sql
-- Description: Fase 4H.2G5. RPC de resumen para /dashboard/inventarios.
--              Calcula KPIs reales, jornadas que requieren atencion y alertas
--              recientes en una sola consulta agregada.
-- Author: Assistant
...
BEGIN
    ...
    FROM (
        SELECT CASE
            WHEN s.task_count > 0 THEN
                pg_catalog.round((t.completed::numeric / s.task_count::numeric) * 100, 1)
            ELSE 0
        END AS progress
        FROM inventarios.sessions s
        CROSS JOIN LATERAL (
            SELECT pg_catalog.count(*) AS completed
    ...
    SELECT CASE
        ...
        ELSE pg_catalog.jsonb_agg(
            pg_catalog.jsonb_build_object(
                ...
                'task_completed_count', s.task_completed_count,
            ...
            ORDER BY
                CASE s.status WHEN 'UNDER_REVIEW' THEN 1 WHEN 'COUNTING' THEN 2 ELSE 3 END,
                s.created_at DESC
        )
    END
    INTO v_attention
