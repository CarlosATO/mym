-- COMV2-08: plan navigation, detail and safe historical versioning.

CREATE OR REPLACE FUNCTION comisiones.protect_issued_settlement()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    IF OLD.status = 'ISSUED' THEN
        RAISE EXCEPTION 'COMV2_ISSUED_SETTLEMENT_IMMUTABLE';
    END IF;
    RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE OR REPLACE FUNCTION comisiones.protect_issued_settlement_line()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM comisiones.settlements st
        WHERE st.company_id = OLD.company_id
          AND st.id = OLD.settlement_id
          AND st.status = 'ISSUED'
    ) THEN
        RAISE EXCEPTION 'COMV2_ISSUED_SETTLEMENT_LINE_IMMUTABLE';
    END IF;
    RETURN COALESCE(NEW, OLD);
END;
$$;

DROP TRIGGER IF EXISTS protect_issued_settlement_change ON comisiones.settlements;
CREATE TRIGGER protect_issued_settlement_change
BEFORE UPDATE OR DELETE ON comisiones.settlements
FOR EACH ROW EXECUTE FUNCTION comisiones.protect_issued_settlement();

DROP TRIGGER IF EXISTS protect_issued_settlement_line_change ON comisiones.settlement_lines;
CREATE TRIGGER protect_issued_settlement_line_change
BEFORE UPDATE OR DELETE ON comisiones.settlement_lines
FOR EACH ROW EXECUTE FUNCTION comisiones.protect_issued_settlement_line();

CREATE OR REPLACE FUNCTION comisiones.list_family_fixed_plans(p_company_id uuid)
RETURNS TABLE (
    id uuid,
    supplier_id uuid,
    supplier_name text,
...
AS $$
    SELECT cp.id, cp.supplier_id,
           COALESCE(NULLIF(btrim(s.business_name), ''), NULLIF(btrim(s.fantasy_name), '')),
           cp.plan_code, cp.version_no, cp.plan_type, cp.valid_from, cp.valid_to,
           cp.status, cp.active, cp.supersedes_plan_id,
           EXISTS (
               SELECT 1 FROM comisiones.settlement_lines sl
               JOIN comisiones.settlements st
                 ON st.company_id = sl.company_id AND st.id = sl.settlement_id
               WHERE sl.company_id = cp.company_id
                 AND sl.plan_id = cp.id
                 AND st.status = 'ISSUED'
           )
    FROM comisiones.commission_plans cp
    JOIN adquisiciones.suppliers s
      ON s.company_id = cp.company_id AND s.id = cp.supplier_id
    WHERE cp.company_id = p_company_id
      AND cp.plan_type = 'FAMILY_FIXED_PERCENT'
    ORDER BY cp.valid_from DESC, cp.version_no DESC, cp.id;
$$;

CREATE OR REPLACE FUNCTION comisiones.get_family_fixed_plan(
    p_company_id uuid,
    p_plan_id uuid
)
RETURNS jsonb
...
AS $$
    SELECT jsonb_build_object(
        ...
        'supplier_name', COALESCE(NULLIF(btrim(s.business_name), ''), NULLIF(btrim(s.fantasy_name), '')),
        'plan_code', cp.plan_code,
        'version_no', cp.version_no,
        'plan_type', cp.plan_type,
        'valid_from', cp.valid_from,
        'valid_to', cp.valid_to,
        'status', cp.status,
        'active', cp.active,
        'supersedes_plan_id', cp.supersedes_plan_id,
        'has_issued_usage', EXISTS (
            SELECT 1 FROM comisiones.settlement_lines sl
            JOIN comisiones.settlements st
              ON st.company_id = sl.company_id AND st.id = sl.settlement_id
            WHERE sl.company_id = cp.company_id
        ...
        'rates', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
                'family_bsale_product_type_id', fr.family_bsale_product_type_id,
                'family_name_snapshot', fr.family_name_snapshot,
                'percentage', fr.percentage
            ) ORDER BY fr.family_name_snapshot)
            FROM comisiones.commission_plan_family_rates fr
            WHERE fr.company_id = cp.company_id AND fr.plan_id = cp.id
        ), '[]'::jsonb)
    )
    FROM comisiones.commission_plans cp
    JOIN adquisiciones.suppliers s
...
BEGIN
    ...
    FOR v_rate IN SELECT value FROM jsonb_array_elements(p_rates) LOOP
        ...
        v_family_name := NULLIF(btrim(v_rate ->> 'family_name_snapshot'), '');
        IF v_family_name IS NULL THEN v_family_name := NULLIF(btrim(v_rate ->> 'family_name'), ''); END IF;
    ...
    ELSE
        ...
        IF v_issued AND (v_rates_changed OR p_valid_from <> v_plan.valid_from) THEN
            ...
            VALUES (p_company_id, p_supplier_id, btrim(p_plan_code), v_version, 'FAMILY_FIXED_PERCENT', p_valid_from, p_valid_to, 'ACTIVE', true, v_plan.id, 'Nueva versión posterior a comisión ISSUED', v_actor, v_actor)
            RETURNING * INTO v_plan;
            v_new_plan_id := v_plan.id;
        ELSE
    ...
    DELETE FROM comisiones.commission_plan_family_rates WHERE company_id = p_company_id AND plan_id = v_new_plan_id;
    FOR v_rate IN SELECT value FROM jsonb_array_elements(p_rates) LOOP
        v_family_id := (v_rate ->> 'family_bsale_product_type_id')::integer;
        ...
        VALUES (p_company_id, v_new_plan_id, v_family_id, v_family_name, v_percentage, v_actor, v_actor);
    END LOOP;
    RETURN QUERY SELECT v_new_plan_id, (SELECT cp.plan_code FROM comisiones.commission_plans cp WHERE cp.id = v_new_plan_id), (SELECT cp.version_no FROM comisiones.commission_plans cp WHERE cp.id = v_new_plan_id);
END;
$$;

GRANT EXECUTE ON FUNCTION comisiones.list_family_fixed_plans(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION comisiones.get_family_fixed_plan(uuid, uuid) TO authenticated, service_role;
