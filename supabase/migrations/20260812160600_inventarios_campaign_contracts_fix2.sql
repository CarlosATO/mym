-- Corrección final de contratos del Informe Global del Inventario (V1).
--
-- Reemplaza list_inventory_campaign_variances y
-- get_inventory_campaign_review_summary con casts numéricos explícitos en la
-- valorización (coalesce(numeric, integer) no existe en PostgreSQL) y el FROM
-- computado que faltaba en los KPIs de stock.
--
...
BEGIN
    ...
    base AS (
        ...
        FROM theoretical t
        UNION ALL
        SELECT ph.bsale_variant_id, NULL::uuid, NULL::text, NULL::text,
    ...
    SELECT
        ...
        pg_catalog.coalesce(pg_catalog.sum(c.theoretical_quantity), 0),
        pg_catalog.coalesce(pg_catalog.sum(c.physical_quantity), 0),
        ...
        pg_catalog.coalesce(pg_catalog.sum(coalesce(c.difference_quantity, 0::numeric) * coalesce(c.unit_cost, 0::numeric)), 0::numeric),
        pg_catalog.coalesce(pg_catalog.sum(pg_catalog.abs(coalesce(c.difference_quantity, 0::numeric) * coalesce(c.unit_cost, 0::numeric))), 0::numeric)
