-- MERMAS: motor transaccional de venta interna a trabajadores.

CREATE TABLE mermas.internal_sale_correlatives (
  company_id uuid NOT NULL REFERENCES core.companies(id) ON DELETE CASCADE,
  sale_year integer NOT NULL,
  next_value bigint NOT NULL DEFAULT 1 CHECK (next_value > 0),
  PRIMARY KEY (company_id, sale_year)
);

CREATE TABLE mermas.internal_sales (
  ...
  employee_rut_snapshot text NOT NULL,
  status text NOT NULL DEFAULT 'PENDING_RENDITION'
    CHECK (status IN ('PENDING_RENDITION', 'RENDERED', 'REVERSED')),
...
CREATE TABLE mermas.internal_sale_lines (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  sale_id uuid NOT NULL REFERENCES mermas.internal_sales(id) ON DELETE RESTRICT,
  company_id uuid NOT NULL REFERENCES core.companies(id) ON DELETE RESTRICT,
  bsale_variant_id integer NOT NULL,
  sku_snapshot text NOT NULL,
  product_name_snapshot text NOT NULL,
  quantity numeric(14,3) NOT NULL CHECK (quantity > 0),
  average_cost_snapshot numeric(14,2) NOT NULL CHECK (average_cost_snapshot > 0),
  markup_percent_snapshot numeric(8,3) NOT NULL CHECK (markup_percent_snapshot >= 0),
  worker_unit_price_snapshot numeric(14,0) NOT NULL CHECK (worker_unit_price_snapshot > 0),
  line_total numeric(14,0) NOT NULL CHECK (line_total > 0),
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE mermas.internal_sale_lot_allocations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES core.companies(id) ON DELETE RESTRICT,
  sale_id uuid NOT NULL REFERENCES mermas.internal_sales(id) ON DELETE RESTRICT,
  sale_line_id uuid NOT NULL REFERENCES mermas.internal_sale_lines(id) ON DELETE RESTRICT,
  movement_id uuid NOT NULL REFERENCES mermas.movements(id) ON DELETE RESTRICT,
...
ALTER TABLE mermas.movements
  ADD COLUMN internal_sale_id uuid REFERENCES mermas.internal_sales(id) ON DELETE RESTRICT,
  ADD COLUMN internal_sale_line_id uuid REFERENCES mermas.internal_sale_lines(id) ON DELETE RESTRICT;

CREATE INDEX mermas_internal_sales_employee_month_idx
  ON mermas.internal_sales(company_id, employee_id, created_at)
  WHERE status <> 'REVERSED';
CREATE INDEX mermas_internal_sale_lines_sale_idx
  ON mermas.internal_sale_lines(company_id, sale_id);
CREATE INDEX mermas_internal_sale_lots_sale_line_idx
  ON mermas.internal_sale_lot_allocations(company_id, sale_line_id);
CREATE INDEX mermas_movements_internal_sale_idx
  ON mermas.movements(company_id, internal_sale_id)
  WHERE internal_sale_id IS NOT NULL;

ALTER TABLE mermas.internal_sales ENABLE ROW LEVEL SECURITY;
ALTER TABLE mermas.internal_sale_lines ENABLE ROW LEVEL SECURITY;
ALTER TABLE mermas.internal_sale_lot_allocations ENABLE ROW LEVEL SECURITY;

CREATE POLICY mermas_internal_sales_select ON mermas.internal_sales FOR SELECT TO authenticated
  USING (core.has_company_access(auth.uid(), company_id) AND portal.has_permission('logistica.mermas.view'));
CREATE POLICY mermas_internal_sale_lines_select ON mermas.internal_sale_lines FOR SELECT TO authenticated
  USING (core.has_company_access(auth.uid(), company_id) AND portal.has_permission('logistica.mermas.view'));
CREATE POLICY mermas_internal_sale_lots_select ON mermas.internal_sale_lot_allocations FOR SELECT TO authenticated
  USING (core.has_company_access(auth.uid(), company_id) AND portal.has_permission('logistica.mermas.view'));

GRANT SELECT ON mermas.internal_sales, mermas.internal_sale_lines,
  mermas.internal_sale_lot_allocations TO authenticated;
GRANT SELECT, INSERT, UPDATE ON mermas.internal_sales, mermas.internal_sale_lines,
  mermas.internal_sale_lot_allocations TO service_role;
GRANT ALL ON mermas.internal_sale_correlatives TO service_role;
REVOKE DELETE ON mermas.internal_sales, mermas.internal_sale_lines,
  mermas.internal_sale_lot_allocations FROM service_role;
  ...
  FOR v_key, v_requested_quantity IN SELECT key, value::numeric FROM jsonb_each_text(v_requested) LOOP
    ...
    FOR v_lot IN
      ...
      WHERE sc.company_id = p_company_id AND sc.variant_id = v_variant_id
        AND (sc.expiration_date IS NULL OR sc.expiration_date >= v_today)
        AND sc.available > 0
