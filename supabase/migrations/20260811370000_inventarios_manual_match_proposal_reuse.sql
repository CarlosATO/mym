-- =========================================================================================
-- MIGRATION: M1.5E.19 - Reutilizar propuesta PENDING_REVIEW del mismo producto por barcode
-- =========================================================================================
-- Problema: al contar el mismo producto con el mismo barcode fisico no asociado en otra
-- ubicacion, submit_mobile_manual_match_count rechazaba porque la guarda comparaba
-- ce.snapshot_product_id (identificador por contexto) en vez de la identidad maestra.
-- Correccion:
--  - Determinar "mismo producto" por bsale_variant_id (identidad maestra), no por
--    snapshot_product_id.
--  - Lock de concurrencia por company + barcode (pg_advisory_xact_lock).
--  - Si existe propuesta PENDING_REVIEW del MISMO producto -> reutilizarla (no crear otra)
--    y emitir evento BARCODE_PROPOSAL_REUSED.
--  - Si existe del OTRO producto -> rechazo INV_BARCODE_ALREADY_PROPOSED.
--  - Si no existe -> crear propuesta nueva (comportamiento actual) con BARCODE_PROPOSED.
--  - Si hay multiples del mismo producto -> seleccion segura (la mas antigua) sin crear
--    otra y reportar inconsistencia en metadata.
--  - Evidencia ligada a count_entry_id (no a proposal_id) porque
--    uq_inventarios_evidence_proposal es UNIQUE por propuesta y la foto de cada conteo
--    debe ser trazable individualmente. Se preserva chk_inventarios_evidence_context
--    (un solo contexto: count_entry_id) y el path unico por idempotencia.
-- Unicamente esquema inventarios. No toca datos QA, storage ni maestras.

-- 1) Ampliar tipos de evento con BARCODE_PROPOSAL_REUSED
...
CHECK (event_type IN ('STARTED', 'RESUMED', 'REOPENED', 'REASSIGNED', 'VALIDATED', 'INVALIDATED', 'CANCELLED', 'LOCATION_OPENED', 'COUNT_RECORDED', 'DISCOVERY_RECORDED', 'BARCODE_PROPOSED', 'LOCATION_CLOSED', 'ZONE_COMPLETED', 'BARCODE_PROPOSAL_REUSED'));


-- 2) submit_mobile_manual_match_count corregido
...
BEGIN
    ...
    -- Lock de concurrencia por company + barcode (serializa la creacion/reutilizacion de
    -- propuestas para el mismo barcode en la misma empresa)
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtext('inventarios.manual_match_barcode'),
        pg_catalog.hashtext(v_company_id::text || ':' || v_clean_scanned_code)
    );

    -- Buscar propuestas PENDING_REVIEW del mismo barcode en la empresa (todas las sesiones)
    SELECT
        pg_catalog.count(*),
    ...
    IF v_proposal_count > 0 THEN
        -- Reutilizar la propuesta mas antigua del mismo producto (o reportar inconsistencia)
    ...
    IF v_has_evidence THEN
        -- Evidencia ligada al count_entry (trazabilidad individual por conteo); la propuesta
        -- reutilizada se referencia en el evento. Se preserva chk_inventarios_evidence_context
        -- (un solo contexto) y el path unico por idempotencia.
