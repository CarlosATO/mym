-- Fix the grouped read-model query by evaluating the history evidence per
-- physical payment before aggregation. The READY/CARRY_FORWARD rule is unchanged.
...
BEGIN
    ...
    WITH physical AS (
        ...
        SELECT p.settlement_id, s.settlement_number, s.settlement_date, s.route_guide_id,
               ...
               CASE WHEN bool_or(p.has_prior_closure) THEN 'CARRY_FORWARD' ELSE 'READY' END AS pending_origin_status,
