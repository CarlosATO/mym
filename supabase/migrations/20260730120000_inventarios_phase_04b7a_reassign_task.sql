...
BEGIN
    ...
    INTO v_previous_assignment_count,
    ...
    INTO v_new_participant_count,
         v_new_participant_id
    FROM inventarios.session_participants AS sp
    JOIN portal.users AS u
      ON u.id = sp.user_id
    JOIN core.user_company_access AS uca
      ON uca.user_id = sp.user_id
     AND uca.company_id = sp.company_id
    ...
    IF p_new_user_id = v_previous_user_id THEN
        RAISE EXCEPTION
            USING ERRCODE = 'P0001',
                  MESSAGE = 'INV_TASK_ALREADY_ASSIGNED',
                  DETAIL = pg_catalog.jsonb_build_object(
                      'message', 'La tarea ya esta asignada al usuario indicado.',
                      'retryable', false
                  )::text;
    END IF;
    ...
    SET released_at = v_occurred_at,
        released_by = v_actor_id,
        release_reason = v_reason
    WHERE ta.company_id = p_company_id
      AND ta.id = v_previous_assignment_id;
    ...
    SET current_assignment_id = v_new_assignment_id,
        active_user_id = CASE
            WHEN v_status = 'IN_PROGRESS' THEN p_new_user_id
            ELSE NULL
        END,
        version = t.version + 1,
...
ALTER FUNCTION inventarios.reassign_inventory_task(uuid, uuid, integer, integer, uuid, text, uuid)
    OWNER TO postgres;
