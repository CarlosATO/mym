...
AS $$
  WITH settings AS (
    ...
    SELECT
      ...
      COALESCE(p.description, dd.variant_description) AS product_name,
      psm.supplier_id,
      supplier.business_name AS supplier_name,
      cgp.commission_group_id,
      commission_group.name AS commission_group_name,
      COALESCE(dd.quantity, 0) AS quantity,
    ...
    JOIN integraciones.bsale_documents d
      ON d.company_id = r.company_id AND d.bsale_id = r.bsale_document_id AND d.document_type_id = 5
    JOIN integraciones.bsale_document_details dd
      ON dd.company_id = d.company_id AND dd.bsale_document_id = d.bsale_id
    JOIN integraciones.bsale_document_sellers ds
      ON ds.company_id = d.company_id AND ds.bsale_document_id = d.bsale_id AND ds.is_primary AND ds.seller_bsale_id = p_seller_bsale_id
    JOIN comercial.commission_seller_profiles sp
      ON sp.company_id = ds.company_id AND sp.seller_bsale_id = ds.seller_bsale_id AND sp.is_commissionable AND sp.active
    LEFT JOIN comercial.customer_reporting_profiles crp
      ON crp.company_id = r.company_id AND crp.bsale_client_id = r.bsale_client_id
    JOIN adquisiciones.products p
      ON p.company_id = r.company_id AND p.sku = dd.variant_code AND p.is_active
    JOIN adquisiciones.product_supplier_mappings psm
      ON psm.company_id = r.company_id AND psm.product_id = p.id AND psm.is_active AND psm.is_preferred
    JOIN adquisiciones.suppliers supplier ON supplier.id = psm.supplier_id
    LEFT JOIN comercial.commission_group_products cgp
      ON cgp.company_id = r.company_id AND cgp.product_id = p.id AND cgp.is_active
     AND r.last_payment_date::date >= cgp.valid_from AND (cgp.valid_to IS NULL OR r.last_payment_date::date <= cgp.valid_to)
    ...
    WHERE r.company_id = p_company_id
      ...
      AND COALESCE(crp.is_internal_account, false) = false
      AND COALESCE(crp.is_commissionable, true) = true
      AND NOT EXISTS (
        SELECT 1 FROM comercial.commission_settlement_lines locked
        WHERE locked.company_id = r.company_id AND locked.invoice_line_id = dd.id
          AND locked.line_type IN ('INVOICE', 'HISTORICAL_MARK') AND locked.eligibility_locked_at IS NOT NULL
      )
  ), accumulated AS (
  ...
  SELECT
    a.seller_bsale_id, a.seller_name, a.period_from, p_period_to,
    ...
    COALESCE(NULLIF(a.supplier_name, ''), 'Proveedor sin nombre'),
    a.commission_group_id, a.commission_group_name, a.quantity, a.net_amount, a.net_amount,
