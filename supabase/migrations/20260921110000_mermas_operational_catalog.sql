-- Mermas: resolve request products from the operational catalog.
-- Stock remains sourced from integraciones.bsale_stock_current.
...
SET search_path = pg_catalog, public, core, portal, integraciones, adquisiciones, mermas, storage
AS $$
DECLARE
  v_code text;
...
BEGIN
  ...
  FOR v_line IN SELECT value FROM jsonb_array_elements(p_lines) LOOP
    SELECT p.bsale_variant_id, p.sku, p.description
    INTO v_variant
    FROM adquisiciones.products p
    WHERE p.id = NULLIF(v_line->>'variant_id', '')::uuid
      AND p.company_id = p_company_id
      AND p.is_active = true
      AND p.status = 'ACTIVE'
      AND p.bsale_variant_id IS NOT NULL
      AND p.bsale_variant_state = 0;
    IF NOT FOUND THEN RAISE EXCEPTION 'Producto no encontrado en el catálogo operativo'; END IF;
    ...
    v_stock_key := v_variant.bsale_variant_id::text;
    ...
    VALUES (p_request_id, p_company_id, v_variant.bsale_variant_id, v_variant.sku, v_variant.description, NULL, v_quantity, v_reason, v_expiration, NULLIF(btrim(v_line->>'lot'), ''), NULLIF(btrim(v_line->>'observation'), ''))
