-- Keep the public text contract explicit when source columns are varchar.
CREATE OR REPLACE FUNCTION adquisiciones.get_route_settlement_check_registry(
    ...
    payment_id uuid, cheque_id uuid, customer_name text, customer_rut text,
    customer_tax_id text, check_date date, amount numeric, check_number text,
    bank_name text, guide_id uuid, guide_number text, settlement_id uuid,
    settlement_number text, settlement_received_at timestamptz,
    original_custodian_id uuid, original_custodian_name text, fund_closure_id uuid,
    fund_closure_number text, fund_closure_status text, fund_closure_at timestamptz,
    deposit_id uuid, deposit_reference_number text, deposit_status text,
    deposit_amount numeric, received_at timestamptz, delivered_to_deposit_at timestamptz,
    deposited_at timestamptz, annulled_at timestamptz, annulled_by uuid,
    void_reason text, operational_status text, current_location text,
...
BEGIN
    ...
    WITH active_closure AS (
        SELECT DISTINCT ON (i.payment_id)
            i.payment_id, f.id AS fund_closure_id, f.closure_number::text,
            f.status::text AS fund_closure_status, f.created_at AS fund_closure_at
        FROM adquisiciones.route_fund_closure_items i
        JOIN adquisiciones.route_fund_closures f ON f.id = i.fund_closure_id
        WHERE i.payment_id IS NOT NULL AND i.released_at IS NULL
        ...
        SELECT DISTINCT ON (dc.payment_id)
            dc.payment_id, d.id AS deposit_id, d.reference_number::text,
            d.status::text AS deposit_status, d.amount AS deposit_amount, d.deposit_date
        ...
        SELECT p.id, p.company_id, p.settlement_id, p.customer_bsale_id,
            p.check_date, p.amount_received, p.check_number::text, p.bank_name::text,
            p.custody_user_id, p.received_at, p.voided_at, p.voided_by,
            p.void_reason::text, ac.fund_closure_id, ac.closure_number,
            ac.fund_closure_status, ac.fund_closure_at, ad.deposit_id,
            ad.reference_number, ad.deposit_status, ad.deposit_amount, ad.deposit_date,
            CASE
                WHEN p.voided_at IS NOT NULL OR p.verification_status = 'VOIDED' THEN 'ANULADO'::text
                WHEN ad.deposit_id IS NOT NULL THEN 'DEPOSITADO'::text
                WHEN ac.fund_closure_id IS NOT NULL THEN 'EN_TESORERIA'::text
                ELSE 'CON_CUSTODIO'::text
    ...
    SELECT p.id, p.id,
        COALESCE(NULLIF(string_agg(DISTINCT si.customer_name, ', '), ''), bc.business_name, 'Cliente no disponible')::text,
        NULLIF(btrim(bc.code), '')::text, NULLIF(btrim(bc.code), '')::text,
        p.check_date, p.amount_received, p.check_number, p.bank_name, g.id,
        g.guide_number::text, s.id, s.settlement_number::text, p.received_at,
        p.custody_user_id, NULLIF(btrim(concat_ws(' ', u.nombre, u.apellido)), '')::text,
        p.fund_closure_id, p.closure_number, p.fund_closure_status, p.fund_closure_at,
        p.deposit_id, p.reference_number, p.deposit_status, p.deposit_amount,
        p.received_at, NULL::timestamptz,
        ...
        p.current_location::text, p.current_location::text,
        CASE
            WHEN p.current_location = 'CON_CUSTODIO' THEN NULLIF(btrim(concat_ws(' ', u.nombre, u.apellido)), '')::text
            WHEN p.current_location = 'EN_TESORERIA' THEN 'Tesorería'::text
        END
    FROM registry p
    ...
    GROUP BY p.id, p.company_id, p.settlement_id, p.customer_bsale_id, p.check_date,
        ...
        p.received_at, p.voided_at, p.voided_by, p.void_reason, p.fund_closure_id,
        p.closure_number, p.fund_closure_status, p.fund_closure_at, p.deposit_id,
        p.reference_number, p.deposit_status, p.deposit_amount, p.deposit_date,
        p.current_location, bc.business_name, bc.code, g.id, g.guide_number,
        s.id, s.settlement_number, u.nombre, u.apellido
