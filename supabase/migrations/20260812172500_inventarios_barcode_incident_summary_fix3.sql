-- Corrección forward del resumen de incidencias de códigos (V1, rev3):
--   * CTE agg declarado en el statement de items;
--   * jsonb_agg con ORDER BY en el nivel externo (subconsulta sin agregado).
--
...
BEGIN
    ...
    SELECT pg_catalog.coalesce(pg_catalog.jsonb_agg(sub.row_data ORDER BY sub.row_data ->> 'sku', sub.row_data ->> 'bsale_variant_id'), '[]'::jsonb)
    INTO v_items
    FROM (
