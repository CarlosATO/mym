-- Qualify all worker account read-model identifiers that can collide with
-- RETURNS TABLE columns or PL/pgSQL variables.
...
BEGIN
  ...
  WITH charge_balances AS (
    ...
    SELECT cb.employee_id,
      coalesce(sum(greatest(cb.original_amount - cb.credit_note_amount - cb.active_paid_amount, 0)), 0)::numeric AS official_balance,
      coalesce(sum(greatest(cb.original_amount - cb.active_paid_amount, 0)) FILTER (WHERE cb.source_type = 'MERMA'), 0)::numeric AS merma_balance,
      coalesce(sum(greatest(cb.original_amount - cb.credit_note_amount - cb.active_paid_amount, 0)) FILTER (WHERE cb.source_type = 'BSALE_BOLETA'), 0)::numeric AS bsale_boleta_balance,
      count(*) AS active_charge_count,
      count(*) FILTER (WHERE greatest(cb.original_amount - cb.credit_note_amount - cb.active_paid_amount, 0) > 0) AS open_charge_count,
      max(cb.charge_at) AS last_charge_at
    FROM charge_balances cb GROUP BY cb.employee_id
  ), account_payments AS (
    SELECT wp.employee_id,
      coalesce(sum(wp.amount) FILTER (WHERE wp.status = 'ACTIVE'), 0)::numeric AS active_payments,
      max(wp.submitted_at) FILTER (WHERE wp.status = 'ACTIVE') AS last_payment_at
    FROM mermas.worker_payments wp
    WHERE wp.company_id = p_company_id
    GROUP BY wp.employee_id
  ), account_employees AS (
    SELECT ac.employee_id FROM account_charges ac UNION SELECT ap.employee_id FROM account_payments ap
  )
