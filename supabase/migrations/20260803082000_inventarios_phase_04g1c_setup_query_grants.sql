-- Migration: 20260803082000_inventarios_phase_04g1c_setup_query_grants.sql
-- Description: Fase 4G.1c. RPC de lectura get_inventory_session_setup y grants
--              EXECUTE para authenticated sobre las RPCs de 4G.1.
-- Author: Assistant

-- ============================================================
-- 1. RPC: get_inventory_session_setup
--    Lectura para authenticated con permiso. Devuelve cabecera, snapshot,
--    participantes activos, zonas, ubicaciones, tareas, asignaciones
--    vigentes e indicadores de configuracion pendiente.
-- ============================================================
...
BEGIN
    ...
    v_indicators := pg_catalog.jsonb_build_object(
        ...
        'active_participant_count', pg_catalog.jsonb_array_length(pg_catalog.coalesce(v_participants, '[]'::jsonb)),
    ...
    RETURN pg_catalog.jsonb_build_object(
        'session', v_session,
        'snapshot', v_snapshot,
        'participants', pg_catalog.coalesce(v_participants, '[]'::jsonb),
        'zones', pg_catalog.coalesce(v_zones, '[]'::jsonb),
        'tasks', pg_catalog.coalesce(v_tasks, '[]'::jsonb),
...
-- ============================================================
-- 3. GRANTS EXECUTE AUTHENTICATED (RPCs 4G.1)
-- ============================================================
GRANT EXECUTE ON FUNCTION inventarios.create_inventory_session(uuid, text, text, uuid, integer, text, uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.add_inventory_session_participant(uuid, uuid, uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.revoke_inventory_session_participant(uuid, uuid, uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.create_inventory_session_zone(uuid, uuid, text, text, text, integer, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.add_inventory_zone_location(uuid, uuid, uuid, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION inventarios.create_inventory_task(uuid, uuid, uuid, uuid, uuid) TO authenticated;
