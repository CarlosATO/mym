-- MIGRATION: 20260627213000_add_seller_to_route_guides.sql

-- 1. Add seller_id and seller_name_snapshot to route_guides
ALTER TABLE logistica.route_guides 
ADD COLUMN IF NOT EXISTS seller_id uuid REFERENCES logistica.route_personnel(id),
ADD COLUMN IF NOT EXISTS seller_name_snapshot text;

ALTER TABLE logistica.route_guides 
ALTER COLUMN driver_id DROP NOT NULL,
ALTER COLUMN driver_name_snapshot DROP NOT NULL;

-- 2. Update create_route_person_inline to normalize names
...
DECLARE
    v_id uuid;
    v_norm_name text := trim(upper(regexp_replace(p_person_name, '\s+', ' ', 'g')));
BEGIN
    -- Check for exact duplicate in normalized name
    SELECT id INTO v_id FROM logistica.route_personnel 
    WHERE company_id = p_company_id AND person_name = v_norm_name AND person_type = p_person_type;
    ...
    VALUES (p_company_id, v_norm_name, p_person_type, p_user_id)
    RETURNING id INTO v_id;
    
    RETURN jsonb_build_object('success', true, 'id', v_id);
EXCEPTION
    WHEN unique_violation THEN
        SELECT id INTO v_id FROM logistica.route_personnel WHERE company_id = p_company_id AND lower(person_name) = lower(v_norm_name) AND person_type = p_person_type;
        RETURN jsonb_build_object('success', true, 'id', v_id);
    WHEN OTHERS THEN
...
-- 3. Update create_route_vehicle_inline
CREATE OR REPLACE FUNCTION logistica.create_route_vehicle_inline(p_company_id uuid, p_vehicle_name text, p_user_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
    v_id uuid;
    v_norm_name text := trim(upper(regexp_replace(p_vehicle_name, '\s+', ' ', 'g')));
BEGIN
    SELECT id INTO v_id FROM logistica.route_vehicles 
    WHERE company_id = p_company_id AND vehicle_name = v_norm_name;
    ...
    VALUES (p_company_id, v_norm_name, p_user_id)
    RETURNING id INTO v_id;
    
    RETURN jsonb_build_object('success', true, 'id', v_id);
EXCEPTION
    WHEN unique_violation THEN
        SELECT id INTO v_id FROM logistica.route_vehicles WHERE company_id = p_company_id AND lower(vehicle_name) = lower(v_norm_name);
        RETURN jsonb_build_object('success', true, 'id', v_id);
    WHEN OTHERS THEN
        RETURN jsonb_build_object('success', false, 'error', SQLERRM);
END;
...
-- 4. Update create_delivery_route_inline
CREATE OR REPLACE FUNCTION logistica.create_delivery_route_inline(p_company_id uuid, p_route_name text, p_user_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
    v_id uuid;
    v_norm_name text := trim(upper(regexp_replace(p_route_name, '\s+', ' ', 'g')));
BEGIN
    SELECT id INTO v_id FROM logistica.delivery_routes 
    WHERE company_id = p_company_id AND route_name = v_norm_name;
...
EXCEPTION
    WHEN unique_violation THEN
        SELECT id INTO v_id FROM logistica.delivery_routes WHERE company_id = p_company_id AND lower(route_name) = lower(v_norm_name);
        RETURN jsonb_build_object('success', true, 'id', v_id);
    WHEN OTHERS THEN
        RETURN jsonb_build_object('success', false, 'error', SQLERRM);
END;
...
-- 5. Update create_route_guide_draft to include seller
...
-- 6. Update update_route_guide_draft to include seller
...
-- 7. Update dispatch_route_guide to validate all fields
