-- =========================================================================================
-- MIGRATION: M1.5E.16 - Guarda de al menos un conteo efectivo al cerrar zona
-- =========================================================================================
-- Correccion funcional: NO se exige que todas las ubicaciones esten CLOSED para cerrar una
-- zona. Las ubicaciones PENDING son advertencia de Mobile, no bloqueo Backend.
-- Nueva guarda servidor en complete_my_counting_zone y complete_inventory_task:
--   debe existir al menos una contribucion/conteo efectivo, no invalidado, de la
--   tarea/zona/ciclo que se intenta completar. Si no existe ninguna captura efectiva:
--   INV_TASK_NO_EFFECTIVE_COUNTS - 'Debes registrar al menos un conteo antes de cerrar la zona.'
-- Se usa get_effective_task_contributions (misma fuente de close_inventory_session y
-- validate_inventory_task). task_selected_coverage_ok se mantiene intacto.
-- Unicamente esquema inventarios.
...
BEGIN
    ...
    -- Guarda nueva: al menos una contribucion efectiva de la tarea/zona/ciclo
    SELECT pg_catalog.count(*) INTO v_effective_counts
    FROM inventarios.get_effective_task_contributions(v_company_id, v_session_id, v_task_id) g
    WHERE g.task_cycle = v_task_cycle;
...
BEGIN
    ...
    -- GUARDA NUEVA: al menos una contribucion efectiva de la tarea/zona/ciclo
    SELECT pg_catalog.count(*) INTO v_effective_counts
    FROM inventarios.get_effective_task_contributions(p_company_id, s, p_task_id) g
    WHERE g.task_cycle = c;
    IF v_effective_counts < 1 THEN
        RAISE EXCEPTION USING ERRCODE='P0001', MESSAGE='INV_TASK_NO_EFFECTIVE_COUNTS',
            DETAIL=pg_catalog.jsonb_build_object('message','Debes registrar al menos un conteo antes de cerrar la zona.','retryable',false)::text;
    END IF;
