-- Cancel a fund closure atomically and release its operational claims.
-- Closure items remain as released history; ACTIVE expenses are reusable.
...
BEGIN
    ...
    -- Only ACTIVE, non-voided expenses are reusable. Their amount/status and
    -- the cancelled closure's historical row are otherwise left unchanged.
