-- =========================================================================================
-- MIGRATION: M1.4A - Mobile Open Counting Location
-- =========================================================================================

-- 1. Extend task_events enumeration to include LOCATION_OPENED
...
ALTER TABLE inventarios.task_events ADD CONSTRAINT chk_inventarios_events_type
    CHECK (event_type IN ('STARTED', 'RESUMED', 'REOPENED', 'REASSIGNED', 'VALIDATED', 'INVALIDATED', 'CANCELLED', 'LOCATION_OPENED'));

-- 2. Create the operational location state table (MINIMAL)
CREATE TABLE inventarios.task_locations (
    id pg_catalog.uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id pg_catalog.uuid NOT NULL REFERENCES core.companies(id) ON DELETE RESTRICT,
    session_id pg_catalog.uuid NOT NULL,
    session_zone_id pg_catalog.uuid NOT NULL,
    task_id pg_catalog.uuid NOT NULL,
    session_zone_location_id pg_catalog.uuid NOT NULL,
    status text NOT NULL DEFAULT 'OPEN',
    opened_at timestamptz NOT NULL DEFAULT pg_catalog.now(),
    opened_by pg_catalog.uuid NOT NULL REFERENCES portal.users(id) ON DELETE RESTRICT,
    
    CONSTRAINT fk_inventarios_task_locations_task
        FOREIGN KEY (task_id) REFERENCES inventarios.tasks(id) ON DELETE RESTRICT,
        
    CONSTRAINT fk_inventarios_task_locations_szl
        FOREIGN KEY (session_zone_location_id) REFERENCES inventarios.session_zone_locations(id) ON DELETE RESTRICT,
        
    CONSTRAINT chk_inventarios_task_locations_status
        CHECK (status IN ('OPEN', 'CLOSED'))
);

COMMENT ON TABLE inventarios.task_locations IS
    'Operational state of a counting location within a specific task.';

-- 3. Unique partial index to enforce ONE active location per ACTOR globally
CREATE UNIQUE INDEX uq_inventarios_task_locations_single_open 
    ON inventarios.task_locations(opened_by) 
    WHERE status = 'OPEN';

-- Second protection: one active open status for the exact same location in the same task
CREATE UNIQUE INDEX uq_inventarios_task_locations_unique_open_loc
    ON inventarios.task_locations(task_id, session_zone_location_id)
    WHERE status = 'OPEN';

ALTER TABLE inventarios.task_locations ENABLE ROW LEVEL SECURITY;
GRANT ALL ON inventarios.task_locations TO service_role;
-- NO direct access for authenticated, they use RPC

-- 4. Create the RPC for opening a location
...
BEGIN
    ...
    INSERT INTO inventarios.task_events (
        ...
        v_actor_id, v_occurred_at, p_idempotency_key
    );
...
ALTER FUNCTION inventarios.open_my_counting_location(pg_catalog.uuid, pg_catalog.uuid, pg_catalog.uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION inventarios.open_my_counting_location(pg_catalog.uuid, pg_catalog.uuid, pg_catalog.uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION inventarios.open_my_counting_location(pg_catalog.uuid, pg_catalog.uuid, pg_catalog.uuid) TO authenticated, service_role;
