-- MERMAS: enrich monthly worker account movements with read-only product items.
-- Financial calculations and movement selection intentionally match the original RPC.
...
BEGIN
  ...
  WITH charge_events AS (
    SELECT c.id AS charge_id, c.employee_id, c.source_type, c.source_id,
    ...
    SELECT ce.employee_id, ce.occurred_at,
      ...
      ce.source_type, ce.source_id, ce.reference_number, ce.charge_id, NULL::uuid AS payment_id,
    ...
    SELECT pe.employee_id, pe.occurred_at, pe.movement_type, 'WORKER_PAYMENT'::text,
      NULL::text, pe.reference_number, NULL::uuid, pe.payment_id, pe.amount, pe.balance_effect
      ...
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'movements', coalesce((
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'items', CASE
              WHEN l.source_type = 'MERMA' THEN coalesce((
                SELECT jsonb_agg(jsonb_build_object(
                  'sku', il.sku_snapshot,
                  'product_name', il.product_name_snapshot,
                  'quantity', il.quantity
                ) ORDER BY il.created_at, il.id)
                FROM mermas.internal_sale_lines il
                WHERE il.company_id = p_company_id
                  AND il.sale_id::text = l.source_id
              ), '[]'::jsonb)
              WHEN l.source_type IN ('BSALE_BOLETA', 'BSALE_NOTA_CREDITO') THEN coalesce((
                SELECT jsonb_agg(jsonb_build_object(
                  'sku', coalesce(v.code, d.variant_code),
                  'product_name', coalesce(bp.name, d.variant_description, v.description, 'Producto Bsale'),
                  'quantity', d.quantity
                ) ORDER BY d.line_number, d.id)
                FROM integraciones.bsale_document_details d
                LEFT JOIN integraciones.bsale_variants v
                  ON v.company_id = d.company_id AND v.bsale_id = d.variant_id
                LEFT JOIN integraciones.bsale_products bp
                  ON bp.company_id = d.company_id AND bp.bsale_id = v.bsale_product_id
                WHERE d.company_id = p_company_id
                  AND d.bsale_document_id::text = l.source_id
              ), '[]'::jsonb)
              ELSE '[]'::jsonb
            END
