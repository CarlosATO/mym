-- Contract: only CASH/CHECK expected payment methods belong to a settlement.
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE route_guide_id = p_route_guide_id
          AND invoice_number <> ''
          AND payment_method_normalized IN ('CASH', 'CHECK')
    ) THEN
        RAISE EXCEPTION 'La guía no tiene facturas rendibles (CASH/CHECK).';
    END IF;
    ...
    SELECT
        ...
        'PENDING_PAYMENT', 0, i.amount, true
