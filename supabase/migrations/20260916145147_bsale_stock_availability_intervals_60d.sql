CREATE TABLE IF NOT EXISTS integraciones.bsale_stock_break_intervals_60d (
    ...
    UNIQUE (company_id, office_id, variant_id, start_date)
);

CREATE INDEX IF NOT EXISTS idx_bsale_stock_break_intervals_variant
    ON integraciones.bsale_stock_break_intervals_60d(company_id, office_id, variant_id, start_date);

ALTER TABLE integraciones.bsale_stock_break_intervals_60d ENABLE ROW LEVEL SECURITY;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'integraciones'
          AND tablename = 'bsale_stock_break_intervals_60d'
          AND policyname = 'rls_bsale_stock_break_intervals_select'
    ) THEN
        CREATE POLICY rls_bsale_stock_break_intervals_select
            ON integraciones.bsale_stock_break_intervals_60d FOR SELECT TO authenticated
            USING (core.has_company_access(auth.uid(), company_id));
    END IF;
END;
$$;

GRANT SELECT ON integraciones.bsale_stock_break_intervals_60d TO authenticated;
GRANT ALL ON integraciones.bsale_stock_break_intervals_60d TO service_role;

ALTER TABLE integraciones.bsale_stock_break_summary_60d
    ADD COLUMN IF NOT EXISTS days_with_stock_60d int NOT NULL DEFAULT 0,
    ADD COLUMN IF NOT EXISTS days_unknown_60d int NOT NULL DEFAULT 0;
...
BEGIN
    ...
    WITH points AS (
        ...
        SELECT variant_id, checkpoint_date,
            bool_or(stock_after <= 0) AS has_zero,
            bool_or(stock_after > 0 OR stock_before > 0) AS has_positive,
            bool_or(is_snapshot) AS has_snapshot,
            (array_agg(stock_after ORDER BY is_snapshot DESC, point_order DESC))[1] AS closing_stock,
            bool_or(stock_after > 0) AS has_positive_after
        FROM points
...
AS $$
    ...
    LEFT JOIN LATERAL (
        SELECT jsonb_agg(
            jsonb_build_object('from', intervals.start_date, 'to', intervals.end_date)
            ORDER BY intervals.start_date
        ) AS items
        FROM integraciones.bsale_stock_break_intervals_60d intervals
        WHERE intervals.company_id = summary.company_id
          AND intervals.office_id = summary.office_id
          AND intervals.variant_id = summary.variant_id
    ) ranges ON true
