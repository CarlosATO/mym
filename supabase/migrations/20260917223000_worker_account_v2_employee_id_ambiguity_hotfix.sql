-- Hotfix: qualify CTE employee_id references in the worker account read model.
...
BEGIN
  ...
  WITH charge_balances AS (
    ...
    SELECT cb.employee_id,
      coalesce(sum(greatest(cb.original_amount - cb.credit_note_amount - cb.approved_paid_amount, 0)), 0)::numeric AS official_balance,
      ...
      coalesce(sum(greatest(cb.original_amount - cb.credit_note_amount - cb.approved_paid_amount, 0)) FILTER (WHERE cb.source_type = 'BSALE_BOLETA'), 0)::numeric AS bsale_boleta_balance,
      count(*) AS active_charge_count,
      count(*) FILTER (WHERE greatest(cb.original_amount - cb.credit_note_amount - cb.approved_paid_amount, 0) > 0) AS open_charge_count,
...
GRANT EXECUTE ON FUNCTION mermas.get_worker_accounts_v2(uuid, uuid, text) TO service_role;
