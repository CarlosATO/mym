-- Migration: 20260806123000_inventarios_prepare_operational_responsible.sql
-- Description: Ajusta la regla de participantes de prepare_inventory_session.
--              Antes exigia COUNTER + SUPERVISOR + MANAGER simultaneamente.
--              Ahora exige al menos 1 COUNTER activo y al menos 1 responsable
--              operacional activo entre ADMINISTRATOR, SUPERVISOR o MANAGER.
--              El actor que ejecuta la preparacion debe ser participante activo
--              con rol ADMINISTRATOR, SUPERVISOR o MANAGER.
--              Incluye saneamiento de bug preexistente del hotfix: v_occurred_at
--              se asignaba despues de insertar snapshot_products/snapshot_stocks
--              (NOT NULL en created_at). Ahora se fija al inicio de la operacion.
-- Esquema: inventarios (unico modificado).
-- Author: Assistant

-- ============================================================
-- 1. RPC: prepare_inventory_session (redefinicion vigente)
--    Permiso: inventarios.sessions.configure
--    Actor: participante operacional activo (ADMINISTRATOR | SUPERVISOR | MANAGER)
-- ============================================================
...
BEGIN
    ...
    END IF;
    -- Timestamp unico de la operacion: debe existir antes de insertar snapshot_products/stocks.
    -- (Bug preexistente del hotfix: v_occurred_at se asignaba despues de esos INSERTs.)
    ...
    -- Actor: participante operacional activo (ADMINISTRATOR, SUPERVISOR o MANAGER).
    -- Un COUNTER por si solo no puede preparar la jornada.
    IF NOT EXISTS (
    ...
    -- Validacion de participantes: al menos un COUNTER y un responsable operacional activo
