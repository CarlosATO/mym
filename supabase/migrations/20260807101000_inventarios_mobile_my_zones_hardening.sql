-- Migration: 20260807101000_inventarios_mobile_my_zones_hardening.sql
-- Description: M1.1A - Hardening of list_my_active_counting_zones RPC with strict search_path
-- Author: Assistant
...
DECLARE
    ...
    v_actor_display_name pg_catalog.text;
    v_result pg_catalog.jsonb;
    v_zones_array pg_catalog.jsonb;
    v_zone_count pg_catalog.int4;
BEGIN
    -- 1. Resolver identidad
    v_actor_id := inventarios.require_actor();

    SELECT pg_catalog.btrim(u.nombre || ' ' || u.apellido) INTO v_actor_display_name
    FROM portal.users u
