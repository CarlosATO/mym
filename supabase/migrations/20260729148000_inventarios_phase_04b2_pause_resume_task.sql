CREATE FUNCTION inventarios.pause_inventory_task(
    p_company_id uuid,
    p_task_id uuid,
...
BEGIN
    ...
    v_payload := pg_catalog.jsonb_build_object('operation', 'inventarios.task.pause', 'company_id', p_company_id, 'task_id', p_task_id, 'expected_version', p_expected_version);
    v_operation := inventarios.begin_idempotent_operation(p_company_id, 'inventarios.task.pause', p_idempotency_key, inventarios.compute_request_hash(v_payload));
    ...
    WHERE ta.company_id = p_company_id AND ta.session_id = v_session_id AND ta.task_id = p_task_id AND ta.released_at IS NULL;
    ...
    END IF;
    IF v_current_assignment_id IS DISTINCT FROM v_assignment_id OR v_assignment_user_id IS DISTINCT FROM v_actor_id OR v_assignment_participant_id IS DISTINCT FROM v_participant_id THEN
    ...
    UPDATE inventarios.tasks AS t SET status = 'PAUSED', version = t.version + 1,
        paused_at = v_occurred_at, paused_by = v_actor_id, active_user_id = NULL,
    ...
    VALUES (p_company_id, v_session_id, v_zone_id, p_task_id, v_assignment_id, v_operation_id, 'PAUSED', 'IN_PROGRESS', 'PAUSED', v_version, v_version + 1, v_cycle, v_cycle, v_actor_id, v_occurred_at);
    v_response := pg_catalog.jsonb_build_object('operation', 'inventarios.task.pause', 'entity_id', p_task_id, 'state', 'PAUSED', 'version', v_version + 1, 'cycle_number', v_cycle, 'assignment_id', v_assignment_id, 'event_id', NULL::uuid, 'replayed', false, 'occurred_at', v_occurred_at, 'data', pg_catalog.jsonb_build_object());
    RETURN inventarios.complete_idempotent_operation(p_company_id, v_operation_id, p_task_id, v_response);
END;
$$;

CREATE FUNCTION inventarios.resume_inventory_task(
    p_company_id uuid,
    p_task_id uuid,
    p_expected_version integer,
    p_idempotency_key uuid
...
BEGIN
    ...
    v_payload := pg_catalog.jsonb_build_object('operation', 'inventarios.task.resume', 'company_id', p_company_id, 'task_id', p_task_id, 'expected_version', p_expected_version);
    v_operation := inventarios.begin_idempotent_operation(p_company_id, 'inventarios.task.resume', p_idempotency_key, inventarios.compute_request_hash(v_payload));
    IF v_operation ->> 'mode' = 'REPLAY' THEN RETURN v_operation -> 'response_payload'; END IF;
    v_operation_id := (v_operation ->> 'operation_id')::uuid;
    SELECT t.session_id, t.session_zone_id, t.status, t.version, t.validation_cycle, t.current_assignment_id
    INTO v_session_id, v_zone_id, v_status, v_version, v_cycle, v_current_assignment_id
    ...
    END IF;
    v_occurred_at := pg_catalog.now();
    UPDATE inventarios.tasks AS t SET status = 'IN_PROGRESS', version = t.version + 1,
        paused_at = NULL, paused_by = NULL, active_user_id = v_actor_id,
        updated_at = v_occurred_at, updated_by = v_actor_id
    WHERE t.company_id = p_company_id AND t.id = p_task_id;
    INSERT INTO inventarios.task_events (company_id, session_id, session_zone_id, task_id, event_type, previous_status, next_status, actor_id, next_user_id, cycle, occurred_at, idempotency_key, created_by)
    VALUES (p_company_id, v_session_id, v_zone_id, p_task_id, 'RESUMED', 'PAUSED', 'IN_PROGRESS', v_actor_id, v_actor_id, v_cycle, v_occurred_at, p_idempotency_key, v_actor_id)
    RETURNING id INTO v_event_id;
    INSERT INTO inventarios.task_state_transitions (company_id, session_id, session_zone_id, task_id, assignment_id, operation_idempotency_id, transition_type, previous_status, next_status, previous_version, next_version, previous_cycle, next_cycle, actor_id, occurred_at)
    VALUES (p_company_id, v_session_id, v_zone_id, p_task_id, v_assignment_id, v_operation_id, 'RESUMED', 'PAUSED', 'IN_PROGRESS', v_version, v_version + 1, v_cycle, v_cycle, v_actor_id, v_occurred_at);
    v_response := pg_catalog.jsonb_build_object('operation', 'inventarios.task.resume', 'entity_id', p_task_id, 'state', 'IN_PROGRESS', 'version', v_version + 1, 'cycle_number', v_cycle, 'assignment_id', v_assignment_id, 'event_id', v_event_id, 'replayed', false, 'occurred_at', v_occurred_at, 'data', pg_catalog.jsonb_build_object());
    RETURN inventarios.complete_idempotent_operation(p_company_id, v_operation_id, p_task_id, v_response);
END;
$$;
...
REVOKE ALL ON FUNCTION inventarios.pause_inventory_task(uuid, uuid, integer, uuid)
FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION inventarios.resume_inventory_task(uuid, uuid, integer, uuid)
FROM PUBLIC, anon, authenticated, service_role;
