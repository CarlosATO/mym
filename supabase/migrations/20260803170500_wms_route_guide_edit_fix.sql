-- Migration: 20260803170500_wms_route_guide_edit_fix.sql
-- Description: Fase WMS-RG.1. Corrige coalesce(text,unknown) y formato de audit_logs
--              en edit_route_guide_unsettled.
-- Author: Assistant
...
BEGIN
    ...
    v_reason := pg_catalog.btrim(pg_catalog.coalesce(p_reason, ''::text));
    ...
    FOR v_item IN SELECT * FROM pg_catalog.jsonb_array_elements(p_items) AS it LOOP
        DECLARE
            ...
            v_customer text;
        BEGIN
            v_invoice := pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'invoice_number', ''::text));
            v_customer := pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'customer_name', ''::text));
            ...
            -- Validaciones
            IF v_invoice = '' THEN
                v_errors := v_errors || 'Factura obligatoria'::text;
            END IF;
            IF v_customer = '' THEN
                v_errors := v_errors || 'Cliente obligatorio'::text;
            END IF;
            IF v_amount <= 0 THEN
                v_errors := v_errors || 'Monto invalido'::text;
            END IF;
            IF v_normalized = 'UNKNOWN' THEN
                v_errors := v_errors || 'Forma de pago no reconocida'::text;
            END IF;
            ...
            v_row_id := NULLIF(pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'id', ''::text)), '')::uuid;
            ...
            IF v_row_id IS NULL THEN
                ...
                INSERT INTO logistica.route_guide_items (
                    ...
                    v_customer, pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'customer_address', ''::text)),
                    pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'commune', ''::text)),
                    v_amount, v_item.value ->> 'payment_method_original', v_normalized,
                    logistica.payment_requires_settlement(v_normalized),
                    v_valid, v_errors, pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'notes', ''::text)),
                    'NOT_REQUIRED'
            ...
            ELSE
                ...
                UPDATE logistica.route_guide_items SET
                    ...
                    customer_address = pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'customer_address', ''::text)),
                    commune = pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'commune', ''::text)),
                    amount = v_amount,
                    ...
                    notes = pg_catalog.btrim(pg_catalog.coalesce(v_item.value ->> 'notes', ''::text)),
    ...
    IF v_existing_ids IS NOT NULL THEN
        ...
        LOOP
            IF NOT (v_item.id = ANY(v_kept_ids)) THEN
                ...
                v_items_deleted := v_items_deleted || pg_catalog.jsonb_build_object('id', v_item.id);
            END IF;
        END LOOP;
    END IF;
    ...
    UPDATE logistica.route_guides rg SET
        total_invoices = pg_catalog.coalesce(rec.total_invoices, 0),
        total_amount = pg_catalog.coalesce(rec.total_amount, 0),
        total_cash_expected = pg_catalog.coalesce(rec.cash, 0),
        total_check_expected = pg_catalog.coalesce(rec.chk, 0),
        total_credit = pg_catalog.coalesce(rec.credit, 0),
        total_transfer = pg_catalog.coalesce(rec.transfer, 0),
        total_unknown_payment = pg_catalog.coalesce(rec.unknown, 0),
        error_count = pg_catalog.coalesce(rec.error_count, 0),
        duplicate_count = pg_catalog.coalesce(rec.duplicate_count, 0)
    FROM rec
    WHERE rg.id = p_route_guide_id;
    ...
    UPDATE logistica.route_guides rg SET
        ...
        route_id = pg_catalog.coalesce((p_header ->> 'route_id')::uuid, rg.route_id),
        route_name_snapshot = COALESCE((SELECT d.name FROM logistica.delivery_routes d WHERE d.id = (p_header ->> 'route_id')::uuid), rg.route_name_snapshot),
        vehicle_id = pg_catalog.coalesce((p_header ->> 'vehicle_id')::uuid, rg.vehicle_id),
        vehicle_name_snapshot = COALESCE((SELECT v.name FROM logistica.route_vehicles v WHERE v.id = (p_header ->> 'vehicle_id')::uuid), rg.vehicle_name_snapshot),
        driver_id = pg_catalog.coalesce((p_header ->> 'driver_id')::uuid, rg.driver_id),
        driver_name_snapshot = COALESCE((SELECT p.name FROM logistica.route_personnel p WHERE p.id = (p_header ->> 'driver_id')::uuid), rg.driver_name_snapshot),
        seller_id = pg_catalog.coalesce((p_header ->> 'seller_id')::uuid, rg.seller_id),
        seller_name_snapshot = COALESCE((SELECT p.name FROM logistica.route_personnel p WHERE p.id = (p_header ->> 'seller_id')::uuid), rg.seller_name_snapshot),
        dispatcher_id = pg_catalog.coalesce((p_header ->> 'dispatcher_id')::uuid, rg.dispatcher_id),
        dispatcher_name_snapshot = COALESCE((SELECT p.name FROM logistica.route_personnel p WHERE p.id = (p_header ->> 'dispatcher_id')::uuid), rg.dispatcher_name_snapshot),
        notes = pg_catalog.btrim(pg_catalog.coalesce(p_header ->> 'notes', rg.notes)),
