-- Keep the global user role and company access roles synchronized.
-- The Users screen has one role selector, so a role change is ambiguous for
-- users with more than one active company assignment.
...
BEGIN
  ...
  IF v_role_name IS NULL THEN
    RAISE EXCEPTION 'USER_ROLE_INVALID';
  END IF;
