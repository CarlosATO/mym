-- Make the bulk identity guard distinguish ownership errors from missing identity.
...
BEGIN
    ...
    SELECT * INTO v_existing FROM adquisiciones.route_settlement_bulk_operations
    ...
    CREATE TEMP TABLE bulk_rows (
        item_id uuid NOT NULL, result text NOT NULL, amount numeric(14,2) NOT NULL,
        group_key text NOT NULL, customer_id bigint NOT NULL, bank_name text,
        check_number text, check_date date
    ) ON COMMIT DROP;
    ...
    FOR v_row IN SELECT value FROM jsonb_array_elements(p_rows) LOOP
        ...
        SELECT si.invoice_number, si.customer_bsale_id INTO v_invoice_number, v_customer
        ...
        WHERE si.id = v_item_id AND si.company_id = v_company AND si.settlement_id = p_settlement_id FOR UPDATE;
        IF NOT FOUND THEN RAISE EXCEPTION 'La factura no pertenece a la rendición.'; END IF;
        IF v_customer IS NULL THEN
            RAISE EXCEPTION 'No se puede grabar la factura % porque no se pudo identificar su cliente en Bsale.', v_invoice_number;
        END IF;
    ...
    FOR v_row IN SELECT to_jsonb(r) FROM bulk_rows r ORDER BY item_id LOOP
        v_item_id := (v_row->>'item_id')::uuid; v_result := v_row->>'result'; v_amount := (v_row->>'amount')::numeric;
        SELECT si.expected_amount, si.customer_bsale_id, COALESCE(si.resolution_type, '') INTO v_expected, v_customer, v_payment_method
        ...
        INTO v_confirmed, v_pending FROM adquisiciones.route_settlement_payment_allocations a JOIN adquisiciones.route_settlement_payments p ON p.id=a.payment_id AND p.company_id=a.company_id
