-- Expose whether a pending fund belongs to a settlement with prior CFC history.
-- The current pending pool contains only unclaimed physical payments. A historical
-- closure-item relation for the same settlement, together with the payment having
-- been received before that closure, is the persisted evidence that a remaining
-- component was carried forward (including released/cancelled items).
...
BEGIN
    ...
    WITH physical AS (
        ...
        SELECT p.settlement_id, s.settlement_number, s.settlement_date, s.route_guide_id,
               ...
               CASE WHEN EXISTS (
                   SELECT 1
                   FROM adquisiciones.route_fund_closure_items history_item
                   JOIN adquisiciones.route_fund_closures history_closure ON history_closure.id = history_item.fund_closure_id
                   WHERE history_item.company_id = p_company_id
                     AND history_item.route_settlement_id = p.settlement_id
                     AND p.received_at <= COALESCE(history_closure.closed_at, history_closure.created_at)
               ) THEN 'CARRY_FORWARD' ELSE 'READY' END AS pending_origin_status,
    ...
    SELECT jsonb_build_object(
        ...
        'pending_origin_status', pending_origin_status,
