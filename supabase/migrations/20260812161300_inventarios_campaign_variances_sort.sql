-- Ordenamiento global en list_inventory_campaign_variances (V1).
--
-- Agrega p_sort_by / p_sort_direction con whitelist cerrada y ORDER BY
-- dinamico estatico (CASE por direccion, sin EXECUTE ni concatenacion de
-- input del cliente). Esquema afectado EXCLUSIVAMENTE: inventarios.
--
-- Compatibilidad: los parametros nuevos tienen DEFAULT; los callers que no
-- los envian conservan el comportamiento previo (sort por sku).
