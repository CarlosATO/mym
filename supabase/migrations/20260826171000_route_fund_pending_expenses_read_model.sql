-- Keep ACTIVE route expenses visible in the pending-funds RPC.
...
WITH physical AS (
    ...
    SELECT route_settlement_id,
           COALESCE(sum(amount) FILTER (WHERE status = 'ACTIVE' AND voided_at IS NULL), 0)::numeric(14,2) AS active_expenses
    FROM adquisiciones.route_fund_closure_expenses
    WHERE company_id = p_company_id
    GROUP BY route_settlement_id
), grouped AS (
    SELECT p.settlement_id, s.settlement_number, s.settlement_date,
           ...
           COALESCE(e.active_expenses, 0)::numeric(14,2) AS active_route_expenses,
           ...
           jsonb_agg(jsonb_build_object(
               ...
               'post_settlement_payment_id', p.post_payment_id,
               'customer_bsale_id', p.customer_bsale_id,
               'received_at', p.received_at,
               'amount', p.amount_received,
               'payment_method', p.payment_method_received
    ...
    GROUP BY p.settlement_id, s.settlement_number, s.settlement_date,
             s.route_guide_id, g.guide_number, p.custody_user_id, e.active_expenses
)
