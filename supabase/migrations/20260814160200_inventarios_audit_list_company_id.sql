-- =========================================================================================
-- MIGRATION: M1.5G.2 - Expone company_id en list_my_inventory_audits (contrato ciego Mobile)
-- =========================================================================================
-- Objetivo:
--   Agregar `company_id` como campo raiz de cada auditoria retornada por
--   inventarios.list_my_inventory_audits, para que Mobile pueda invocar los RPC de
--   ejecucion de auditorias (start_inventory_audit, record_inventory_audit_result,
--   list_my_inventory_audit_progress, submit_inventory_audit) que requieren p_company_id.
--
-- Alcance:
--   UNICAMENTE se agrega el campo raiz `company_id` (valor real de inventory_audits.
--   company_id). Se preservan integramente: filtrado por usuario autenticado, autorizacion
--   existente (assigned_user_id + core.user_company_access), estructura de productos,
--   locations, search_scope, estados y el contrato ciego. `company_id` es contexto tecnico
--   de invocacion; NO expone datos administrativos nuevos.
--
-- No se crea ningun RPC nuevo ni se modifican los RPC de ejecucion de auditorias.
...
BEGIN
    ...
    INTO v_audits
    FROM (
        SELECT a.id, a.company_id, a.audit_number, a.status, a.campaign_id,
