-- =========================================================================================
-- MIGRATION: M1.5E.11 - Alcance maestra global en busqueda; deshabilitar descubrimiento
-- =========================================================================================
-- 1) search_master_products: la busqueda consulta TODA la maestra activa oficial de la
--    empresa (adquisiciones.products is_active=true), por nombre y SKU, sin filtrar por
--    zona/bodega/ubicacion/snapshot ni excluir productos activos por inconsistencia del
--    espejo Bsale. Zona/ubicacion/sesion/tarea solo autorizan contextualmente y sirven
--    para calcular in_snapshot/snapshot_product_id contra el snapshot de la ubicacion.
--    Se conserva el fix de coalesce.
-- 2) submit_mobile_manual_match_count: NO se modifica (ya revalida en servidor que el
--    snapshot_product_id pertenece al snapshot; un resultado in_snapshot=false no puede
--    forzarlo).
-- 3) submit_mobile_discovery_count: se deshabilita funcionalmente para esta version,
--    rechazando inmediatamente con INV_OPERATION_NOT_ENABLED antes de cualquier lectura
--    o escritura operativa. Se preservan firma, SECURITY DEFINER, search_path, owner y
--    grants. No se elimina la funcion ni se tocan datos historicos.
...
BEGIN
    ...
    master_rows AS (
        SELECT
            ...
            pg_catalog.coalesce(NULLIF(pg_catalog.btrim(p.description), ''), NULLIF(pg_catalog.btrim(p.short_description), '')) AS name,
            CASE
...
AS $function$
BEGIN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_OPERATION_NOT_ENABLED',
        DETAIL = pg_catalog.jsonb_build_object(
            'message', 'El descubrimiento de productos no está habilitado en esta versión del inventario.',
            'retryable', false
        )::text;
END;
$function$;
