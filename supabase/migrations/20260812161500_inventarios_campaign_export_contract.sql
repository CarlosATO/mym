-- Contrato read-only de exportación del Informe Global del Inventario (V1).
--
-- get_inventory_campaign_export(...):
--   * contributions : todas las contribuciones físicas efectivas de todas las
--     secciones del Inventario, con contexto (bodega/sección, zona, ubicación,
--     contador, método, barcode, fecha/hora) y sku/name del producto.
--   * operational_rows : filas de auditoría del estado operacional
--     (secciones por estado, zonas, ubicaciones, tareas, productos no incluidos,
--     códigos pendientes) para la hoja "Estado operacional" del Excel.
--
-- Reutiliza la cadena efectiva (get_effective_task_contributions) para NO sumar
-- count_entries crudas.
--
