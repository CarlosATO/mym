-- Migration: 20260806122000_inventarios_counting_zone_reassign_fix.sql
-- Fase 4I.3C.7C.4B (correccion prueba manual). Al cancelar una zona, su fila
-- persiste con is_enabled = false y uq_inventarios_zones_code (UNIQUE sin
-- filtro de estado) mantiene ocupado su zone_code. La generacion de codigo
-- contaba solo zonas habilitadas, por lo que la siguiente zona volvia a
-- intentar 'Z1' y fallaba con 23505.
-- Fix: zone_code/priority se derivan del total de filas de la sesion
-- (creadas + canceladas); la cobertura del envelope informa las zonas
-- habilitadas (incluida la nueva).
...
-- ============================================================
-- OWNER, REVOKES Y GRANTS (replican el schema original).
-- ============================================================
