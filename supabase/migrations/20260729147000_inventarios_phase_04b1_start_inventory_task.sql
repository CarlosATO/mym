CREATE FUNCTION inventarios.start_inventory_task(
    p_company_id uuid,
    p_task_id uuid,
...
BEGIN
    ...
    INTO v_task_session_id,
         v_task_session_zone_id,
         v_task_status,
         v_task_version,
         v_task_cycle,
    ...
    v_participant_id := inventarios.require_session_participant(
        p_company_id,
        v_task_session_id,
        'COUNTER'
    );
    ...
    WHERE ta.company_id = p_company_id
      AND ta.session_id = v_task_session_id
    ...
    IF v_current_assignment_id IS DISTINCT FROM v_assignment_id
       ...
       OR v_assignment_participant_id IS DISTINCT FROM v_participant_id THEN
        RAISE EXCEPTION
            USING ERRCODE = 'P0001',
                  MESSAGE = 'INV_ASSIGNMENT_REQUIRED',
                  DETAIL = pg_catalog.jsonb_build_object(
                      'message', 'No tienes una asignacion vigente para esta tarea.',
                      'retryable', false
                  )::text;
    END IF;
...
ALTER FUNCTION inventarios.start_inventory_task(uuid, uuid, integer, uuid)
OWNER TO postgres;

REVOKE ALL ON FUNCTION inventarios.start_inventory_task(uuid, uuid, integer, uuid)
