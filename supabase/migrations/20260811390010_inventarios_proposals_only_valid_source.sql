-- =========================================================================================
-- MIGRATION M1: 202608113a00_inventarios_proposals_only_valid_source.sql
-- =========================================================================================
-- Objetivo: una propuesta PENDING_REVIEW cuyo count_entry_id origen fue invalidado deja de
-- participar en la resolucion operacional Mobile, aunque permanece historica en
-- product_barcode_proposals.
-- Criterio canonico de vigencia (igual a get_effective_count_entries y respaldado por
-- chk_inventarios_counts_invalidation):
--     ce.invalidated_at IS NULL AND ce.invalidated_by IS NULL AND ce.invalidation_reason IS NULL
-- RPC ajustadas (solo su logica de consulta de propuestas; sin cambiar firmas ni envelopes):
--   resolve_my_scanned_barcode
--   get_my_manual_barcode_resolution
--   submit_mobile_manual_match_count
