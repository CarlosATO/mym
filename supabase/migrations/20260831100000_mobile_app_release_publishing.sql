-- Secure publishing contract for the internal Inventarios Android app.

CREATE OR REPLACE FUNCTION portal.is_super_usuario(p_user_id uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION portal.is_super_usuario(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION portal.is_super_usuario(uuid) TO authenticated, service_role;

ALTER TABLE portal.mobile_app_releases
    DROP CONSTRAINT IF EXISTS mobile_app_releases_build_number_check,
    ADD CONSTRAINT mobile_app_releases_build_number_positive CHECK (build_number > 0);

CREATE UNIQUE INDEX IF NOT EXISTS mobile_app_releases_version_build
    ON portal.mobile_app_releases (app_key, version, build_number);

DROP POLICY IF EXISTS mobile_app_releases_admin_select ON portal.mobile_app_releases;
DROP POLICY IF EXISTS mobile_app_releases_admin_insert ON portal.mobile_app_releases;
DROP POLICY IF EXISTS mobile_app_releases_admin_update ON portal.mobile_app_releases;
DROP POLICY IF EXISTS mobile_app_releases_admin_delete ON portal.mobile_app_releases;

CREATE POLICY mobile_app_releases_super_select
    ON portal.mobile_app_releases FOR SELECT
    TO authenticated USING (portal.is_super_usuario());
CREATE POLICY mobile_app_releases_super_insert
    ON portal.mobile_app_releases FOR INSERT
    TO authenticated WITH CHECK (portal.is_super_usuario());
CREATE POLICY mobile_app_releases_super_update
    ON portal.mobile_app_releases FOR UPDATE
    TO authenticated USING (portal.is_super_usuario())
    WITH CHECK (portal.is_super_usuario());
CREATE POLICY mobile_app_releases_super_delete
    ON portal.mobile_app_releases FOR DELETE
    TO authenticated USING (portal.is_super_usuario());

DROP POLICY IF EXISTS mobile_app_releases_storage_insert ON storage.objects;
CREATE POLICY mobile_app_releases_storage_insert
    ON storage.objects FOR INSERT TO authenticated
    WITH CHECK (
        bucket_id = 'mobile-apps'
        AND name ~ '^inventarios/[^/]+[.]apk$'
        AND portal.is_super_usuario()
    );
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 FROM portal.mobile_app_releases
        WHERE app_key = 'inventory_mobile'
