CREATE OR REPLACE FUNCTION inventarios.start_inventory_recount(
    p_company_id uuid,
...
BEGIN
    ...
    PERFORM pg_catalog.pg_advisory_xact_lock(hashtext('inventarios.start_inventory_recount.idempotency'), hashtext(p_company_id::text || ':' || p_idempotency_key::text));
    v_payload := jsonb_build_object('operation','inventarios.recount.start','company_id',p_company_id,'recount_request_id',p_recount_request_id,'expected_status','ASSIGNED');
    v_operation := inventarios.begin_idempotent_operation(p_company_id,'inventarios.recount.start',p_idempotency_key,inventarios.compute_request_hash(v_payload));
    IF v_operation ->> 'mode' = 'REPLAY' THEN RETURN v_operation -> 'response_payload'; END IF;
    v_operation_id := (v_operation ->> 'operation_id')::uuid;
    PERFORM pg_catalog.pg_advisory_xact_lock(hashtext('inventarios.start_inventory_recount'), hashtext(p_company_id::text || ':' || p_recount_request_id::text));
    SELECT rr.session_id, rr.session_zone_id, rr.snapshot_product_id, rr.source_task_id, rr.cycle_number,
           rr.assigned_participant_id, rr.assigned_user_id, rr.assigned_at
    INTO v_session_id, v_zone_id, v_prod_id, v_task_id, v_cycle,
         v_app_id, v_auser_id, v_aat
    ...
    IF v_session_id IS NULL OR v_zone_id IS NULL OR v_prod_id IS NULL OR v_task_id IS NULL OR v_cycle IS NULL OR v_cycle < 1 THEN
    ...
    END IF;
    IF v_app_id IS NULL OR v_auser_id IS NULL OR v_aat IS NULL THEN
        RAISE EXCEPTION USING ERRCODE='P0001', MESSAGE='INV_CONCURRENT_MODIFICATION', DETAIL=jsonb_build_object('message','Se detecto una modificacion concurrente.','retryable',true)::text;
    END IF;
    ...
    SET status = 'IN_PROGRESS', started_at = v_started_at,
        updated_at = v_started_at, updated_by = v_actor_id
    ...
    v_resp := jsonb_build_object('operation','inventarios.recount.start','entity_id',p_recount_request_id,'state','IN_PROGRESS','version',NULL::integer,'cycle_number',v_cycle,'assignment_id',NULL::uuid,'event_id',NULL::uuid,'replayed',false,'occurred_at',v_started_at,'data',jsonb_build_object('assigned_participant_id',v_app_id,'assigned_user_id',v_auser_id,'assigned_at',v_aat,'started_at',v_started_at,'session_id',v_session_id,'session_zone_id',v_zone_id,'snapshot_product_id',v_prod_id,'source_task_id',v_task_id,'ordinal',(SELECT rr.ordinal FROM inventarios.recount_requests rr WHERE rr.id = p_recount_request_id)));
    RETURN inventarios.complete_idempotent_operation(p_company_id, v_operation_id, p_recount_request_id, v_resp);
END;
$$;
ALTER FUNCTION inventarios.start_inventory_recount(uuid, uuid, text, uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION inventarios.start_inventory_recount(uuid, uuid, text, uuid) FROM PUBLIC, anon, authenticated, service_role;
