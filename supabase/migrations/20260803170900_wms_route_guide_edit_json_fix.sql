-- Migration: 20260803170900_wms_route_guide_edit_json_fix.sql
-- Description: Fase WMS-RG.1. Corrige concatenacion jsonb con text en
--              validation_errors (invalid input syntax for type json).
-- Author: Assistant
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM pg_catalog.jsonb_array_elements(p_items) AS it LOOP
        ...
        BEGIN
            ...
            -- Validaciones
            IF v_invoice = '' THEN
                v_errors := v_errors || '"Factura obligatoria"'::jsonb;
            END IF;
            IF v_customer = '' THEN
                v_errors := v_errors || '"Cliente obligatorio"'::jsonb;
            END IF;
            IF v_amount <= 0 THEN
                v_errors := v_errors || '"Monto invalido"'::jsonb;
            END IF;
            IF v_normalized = 'UNKNOWN' THEN
                v_errors := v_errors || '"Forma de pago no reconocida"'::jsonb;
            END IF;
