-- Migration: 20260903200000_inventarios_mobile_rejected_recount_contracts.sql
-- Description: Expone al Mobile los reconteos creados por el backend al rechazar
--              una incidencia de codigo (inventarios.recount_requests) mediante
--              dos contratos: listar los reconteos pendientes asignados al
--              usuario autenticado y registrar/completar un reconteo sin foto.
--              Reutiliza la estructura persistida por 20260903190000 (recount_requests).
--              No modifica ningun contrato anterior (rechazo, conteo normal,
--              incidencias, aliases, cierre de zona, cobertura, start/pause/resume,
--              auditorias, snapshots, Mobile/UI).

BEGIN;

-- ============================================================
-- 1. LISTA: reconteos pendientes asignados al usuario autenticado
--    Solo tareas en ASSIGNED/IN_PROGRESS y asignadas al actor.
--    Entrega id, producto (id/snapshot_product_id/SKU/nombre),
--    ubicacion, zona, sesion/campana, cantidad rechazada de referencia y estado.
-- ============================================================
...
BEGIN
    ...
    FROM (
        ...
        WHERE rr.company_id = p_company_id
          AND rr.assigned_user_id = v_actor_id
...
--    Valida asignacion, campana operativa y permite la operacion
--    aunque zona/seccion este cerrada. Idempotente por clave.
-- ============================================================
...
BEGIN
    ...
    IF p_company_id IS NULL OR p_recount_request_id IS NULL OR p_quantities IS NULL
    ...
    END IF;
    v_devid := pg_catalog.btrim(pg_catalog.coalesce(p_device_id, ''));
    ...
    -- 2. Idempotencia (reintento del mismo payload no duplica conteos).
    v_payload := pg_catalog.jsonb_build_object(
        'operation','inventarios.recount.mobile_submit','company_id',p_company_id,
    ...
    v_operation := inventarios.begin_idempotent_operation(
        p_company_id, 'inventarios.recount.mobile_submit', p_idempotency_key,
    ...
    PERFORM pg_catalog.pg_advisory_xact_lock(
        ...
        pg_catalog.hashtext(p_company_id::text || ':' || p_recount_request_id::text)
    );

    -- 3. Reconteo existente.
    ...
    FROM inventarios.recount_requests rr
    WHERE rr.company_id = p_company_id AND rr.id = p_recount_request_id
    FOR UPDATE;
    IF NOT FOUND THEN
    ...
    -- 5. La campana debe seguir operativa. Zona/seccion cerrada no bloquea.
    ...
    -- 6. Variante BSale del producto snapshot.
    ...
    IF v_variant_id IS NULL AND v_source_ce_id IS NOT NULL THEN
        ...
        WHERE ce.company_id = p_company_id AND ce.id = v_source_ce_id;
    END IF;
    ...
    -- 7. Ubicacion del snapshot dentro de la zona.
    ...
    -- 8. Guarda anti-duplicados: un reconteo ya resuelto no debe sumar un segundo conteo.
    IF EXISTS (
    ...
    -- 9. Marcar inicio si la tarea estaba asignada y no iniciada.
    ...
    v_cap := pg_catalog.coalesce(p_captured_at, v_serv);
    v_cap := pg_catalog.least(v_cap, v_serv);
    ...
    -- 10. Registrar el NUEVO count_entry (el rechazado permanece historico).
    INSERT INTO inventarios.count_entries (
        ...
        p_company_id, v_session_id, v_snapshot_id, v_zone_id, v_task_id, v_cycle,
    ...
    -- 11. Resolver exactamente la tarea de reconteo.
    v_completed_at := pg_catalog.greatest(pg_catalog.now(), v_started_at);
    UPDATE inventarios.recount_requests
    ...
    RETURN inventarios.complete_idempotent_operation(
        p_company_id, v_operation_id, p_recount_request_id, v_resp
    );
END;
$function$;
...
-- ============================================================
ALTER FUNCTION inventarios.list_my_pending_recount_tasks(uuid) OWNER TO postgres;
ALTER FUNCTION inventarios.submit_my_recount_task(uuid, uuid, jsonb, text, text, uuid, timestamptz, text) OWNER TO postgres;

REVOKE ALL ON FUNCTION inventarios.list_my_pending_recount_tasks(uuid) FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION inventarios.submit_my_recount_task(uuid, uuid, jsonb, text, text, uuid, timestamptz, text) FROM PUBLIC, anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION inventarios.list_my_pending_recount_tasks(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION inventarios.submit_my_recount_task(uuid, uuid, jsonb, text, text, uuid, timestamptz, text) TO authenticated, service_role;
