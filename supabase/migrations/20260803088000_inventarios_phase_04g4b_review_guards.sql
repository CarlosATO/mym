-- Migration: 20260803088000_inventarios_phase_04g4b_review_guards.sql
-- Description: Fase 4G.4b. Guardas de sesion UNDER_REVIEW en invalidate y
--              reopen_inventory_task. Ambas operan sobre tareas COMPLETED y
--              pertenecen a la fase de revision.
-- Author: Assistant

-- ============================================================
-- 1. invalidate_inventory_task: agrega guarda UNDER_REVIEW
-- ============================================================
...
BEGIN
    ...
    v_payload := pg_catalog.jsonb_build_object(
        'operation', 'inventarios.task.invalidate', 'company_id', p_company_id,
        'task_id', p_task_id, 'expected_version', p_expected_version,
        'expected_cycle', p_expected_cycle, 'reason', v_reason);
    v_idempotency_result := inventarios.begin_idempotent_operation(
        p_company_id, 'inventarios.task.invalidate', p_idempotency_key,
    ...
    INTO v_session_id, v_session_zone_id, v_status, v_version, v_cycle,
         v_validation_event_id, v_validated_at, v_validated_by
    ...
    END IF;
    PERFORM inventarios.require_session_review(p_company_id, v_session_id);
    ...
    IF v_validation_event_id IS NULL THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_TASK_NOT_VALIDATED',
            DETAIL = pg_catalog.jsonb_build_object('message', 'La tarea no tiene una validacion vigente.', 'retryable', false)::text;
    END IF;
    SELECT e.company_id, e.session_id, e.session_zone_id, e.task_id, e.cycle,
           e.event_type, e.actor_id, e.occurred_at
    INTO v_event_company_id, v_event_session_id, v_event_session_zone_id,
         v_event_task_id, v_event_cycle, v_event_type, v_event_actor_id, v_event_occurred_at
    ...
    VALUES (p_company_id, v_session_id, v_session_zone_id, p_task_id, 'INVALIDATED',
        v_actor_id, v_cycle, v_occurred_at, v_reason, p_idempotency_key,
        pg_catalog.jsonb_build_object('invalidated_validation_event_id', v_validation_event_id, 'reason', v_reason),
        v_actor_id)
    RETURNING e.id INTO v_invalidation_event_id;
    UPDATE inventarios.tasks AS t
    SET current_validation_event_id = NULL, validated_at = NULL, validated_by = NULL,
        version = t.version + 1, updated_at = v_occurred_at, updated_by = v_actor_id
    WHERE t.company_id = p_company_id AND t.id = p_task_id;
    v_response := pg_catalog.jsonb_build_object(
        'operation', 'inventarios.task.invalidate', 'entity_id', p_task_id,
        'state', 'COMPLETED', 'version', v_version + 1, 'cycle_number', v_cycle,
        'assignment_id', NULL::uuid, 'event_id', v_invalidation_event_id,
        'replayed', false, 'occurred_at', v_occurred_at,
        'data', pg_catalog.jsonb_build_object(
            'invalidated_validation_event_id', v_validation_event_id, 'reason', v_reason));
    RETURN inventarios.complete_idempotent_operation(p_company_id, v_operation_id, p_task_id, v_response);
END;
$$;

-- ============================================================
-- 2. reopen_inventory_task: agrega guarda UNDER_REVIEW
-- ============================================================
...
BEGIN
    ...
    PERFORM pg_catalog.pg_advisory_xact_lock(
        ...
        pg_catalog.hashtext(p_company_id::text || ':' || p_task_id::text));
    v_payload := pg_catalog.jsonb_build_object(
        'operation', 'inventarios.task.reopen', 'company_id', p_company_id,
        'task_id', p_task_id, 'expected_version', p_expected_version,
        'expected_cycle', p_expected_cycle, 'reason', v_reason);
    v_operation := inventarios.begin_idempotent_operation(
        p_company_id, 'inventarios.task.reopen', p_idempotency_key,
        inventarios.compute_request_hash(v_payload));
    ...
    SELECT t.session_id, t.session_zone_id, t.status, t.version, t.validation_cycle,
           t.current_assignment_id, t.current_validation_event_id, t.validated_at, t.validated_by
    INTO v_session_id, v_session_zone_id, v_status, v_version, v_previous_cycle,
         v_current_assignment_id, v_previous_validation_event_id, v_validated_at, v_validated_by
    FROM inventarios.tasks AS t
    WHERE t.company_id = p_company_id AND t.id = p_task_id FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_NOT_FOUND',
    ...
    IF v_status <> 'COMPLETED' THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_TASK_INVALID_STATE',
            DETAIL = pg_catalog.jsonb_build_object('message', 'La tarea no permite esta operacion en su estado actual.', 'retryable', false)::text;
    END IF;
    IF v_version <> p_expected_version OR v_previous_cycle <> p_expected_cycle THEN
        RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INV_CONCURRENT_MODIFICATION',
            DETAIL = pg_catalog.jsonb_build_object('message', 'La tarea fue modificada por otra operacion.', 'retryable', true)::text;
    END IF;
    PERFORM inventarios.require_session_participant(p_company_id, v_session_id, 'SUPERVISOR');
    ...
    INTO v_assignment_count, v_assignment_id, v_assigned_user_id
    ...
    IF v_assignment_count <> 1 OR v_current_assignment_id IS DISTINCT FROM v_assignment_id
    ...
    v_occurred_at := pg_catalog.now();
    INSERT INTO inventarios.task_events AS e (company_id, session_id, session_zone_id,
        task_id, event_type, previous_status, next_status, actor_id, next_user_id,
        cycle, occurred_at, reason, idempotency_key, technical_metadata, created_by)
    VALUES (p_company_id, v_session_id, v_session_zone_id, p_task_id, 'REOPENED',
        'COMPLETED', 'IN_PROGRESS', v_actor_id, v_assigned_user_id, v_new_cycle,
        v_occurred_at, v_reason, p_idempotency_key,
        pg_catalog.jsonb_build_object('previous_cycle', v_previous_cycle,
            'previous_validation_event_id', v_previous_validation_event_id, 'reason', v_reason),
        v_actor_id)
    RETURNING e.id INTO v_reopened_event_id;
    UPDATE inventarios.tasks AS t
    SET status = 'IN_PROGRESS', validation_cycle = v_new_cycle,
        current_validation_event_id = NULL, validated_at = NULL, validated_by = NULL,
        opened_at = v_occurred_at, paused_at = NULL, paused_by = NULL,
        completed_at = NULL, completed_by = NULL, active_user_id = v_assigned_user_id,
        version = t.version + 1, updated_at = v_occurred_at, updated_by = v_actor_id
    WHERE t.company_id = p_company_id AND t.id = p_task_id;
    INSERT INTO inventarios.task_state_transitions (company_id, session_id,
        session_zone_id, task_id, assignment_id, operation_idempotency_id,
        transition_type, previous_status, next_status, previous_version, next_version,
        previous_cycle, next_cycle, actor_id, reason, occurred_at)
    VALUES (p_company_id, v_session_id, v_session_zone_id, p_task_id, v_assignment_id,
        v_operation_id, 'REOPENED', 'COMPLETED', 'IN_PROGRESS', v_version, v_version + 1,
        v_previous_cycle, v_new_cycle, v_actor_id, v_reason, v_occurred_at);
    v_response := pg_catalog.jsonb_build_object(
        'operation', 'inventarios.task.reopen', 'entity_id', p_task_id,
        'state', 'IN_PROGRESS', 'version', v_version + 1, 'cycle_number', v_new_cycle,
        'assignment_id', v_assignment_id, 'event_id', v_reopened_event_id,
        'replayed', false, 'occurred_at', v_occurred_at,
        'data', pg_catalog.jsonb_build_object('previous_cycle', v_previous_cycle,
            'previous_validation_event_id', v_previous_validation_event_id, 'reason', v_reason));
    RETURN inventarios.complete_idempotent_operation(p_company_id, v_operation_id, p_task_id, v_response);
END;
$$;
...
-- 3. OWNER Y REVOKES
-- ============================================================
ALTER FUNCTION inventarios.invalidate_inventory_task(uuid, uuid, integer, integer, text, uuid) OWNER TO postgres;
ALTER FUNCTION inventarios.reopen_inventory_task(uuid, uuid, integer, integer, text, uuid) OWNER TO postgres;

REVOKE ALL ON FUNCTION inventarios.invalidate_inventory_task(uuid, uuid, integer, integer, text, uuid) FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION inventarios.reopen_inventory_task(uuid, uuid, integer, integer, text, uuid) FROM PUBLIC, anon, authenticated, service_role;
