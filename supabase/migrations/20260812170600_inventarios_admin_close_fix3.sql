-- Corrección forward del RPC de cierre (V1, rev 3):
--   * v_has_official bigint (no boolean);
--   * official_versions.item_count calculado antes del INSERT (CHECK exige >=1);
--   * reviewed_at fijado al cerrar sesiones COUNTING/UNDER_REVIEW directamente a
--     APPROVED (CHECK chk_inventarios_sessions_lifecycle_dates exige approved_at
--     con reviewed_at previo).
--
