-- =========================================================================================
-- MIGRATION: M1.5E.1 - Neutralidad de rol en asignaciones al preparar jornada
-- =========================================================================================
...
BEGIN
    ...
    WHERE ta.company_id = p_company_id AND ta.session_id = p_session_id
      ...
      AND t.cancelled_at IS NULL AND t.superseded_at IS NULL
      AND (
        sp.functional_role NOT IN ('COUNTER', 'SUPERVISOR', 'ADMINISTRATOR', 'MANAGER')
    ...
    SELECT sp.company_id, sp.snapshot_id, sp.id, v_bsale_office_id, bsc.quantity, bsc.bsale_sync_run_id, bsc.synced_at, v_occurred_at, v_actor_id
    FROM inventarios.snapshot_products sp
    ...
    WHERE sp.company_id = p_company_id AND sp.snapshot_id = v_snapshot_id AND bsc.quantity >= 0
