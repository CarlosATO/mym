-- Migration: 20260804120000_inventarios_campaign_import_validation.sql
-- Description: Fase 4I.3C.2B. Validacion semantica de importaciones de campana.
-- Author: Assistant

ALTER TABLE inventarios.stock_import_row_issues ALTER COLUMN row_id DROP NOT NULL;
ALTER TABLE inventarios.stock_import_row_issues ADD COLUMN IF NOT EXISTS field text;
ALTER TABLE inventarios.stock_import_row_issues ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb;
...
DECLARE
    ...
    v_has_error boolean; v_has_warning boolean; v_sku text; v_sku_norm text; v_barcode text; v_entered_name text; v_site_code text; v_site_norm text; v_location_code text; v_location_norm text; v_quantity_text text; v_cost_text text; v_quantity numeric; v_cost numeric;
    ...
    v_file_issue_level text; v_file_issue_code text; v_file_issue_field text; v_file_issue_message text; v_file_issue_metadata jsonb; v_file_issue_row_index integer; v_row_issues jsonb;
BEGIN
    ...
    FOR v_row IN SELECT value FROM pg_catalog.jsonb_array_elements(p_rows) LOOP
        ...
        v_sku := pg_catalog.btrim(COALESCE(v_row->>'sku', '')); v_barcode := pg_catalog.btrim(COALESCE(v_row->>'barcode', '')); v_entered_name := pg_catalog.btrim(COALESCE(v_row->>'entered_name', '')); v_site_code := pg_catalog.btrim(COALESCE(v_row->>'entered_site_code', v_row->>'site_code', '')); v_location_code := pg_catalog.btrim(COALESCE(v_row->>'entered_location_code', v_row->>'location_code', '')); v_quantity_text := pg_catalog.btrim(COALESCE(v_row->>'quantity', '')); v_cost_text := pg_catalog.btrim(COALESCE(v_row->>'cost', v_row->>'unit_cost', ''));
        ...
        IF v_sku = '' AND v_site_code = '' AND v_location_code = '' AND v_quantity_text = '' AND v_cost_text = '' THEN CONTINUE; END IF;
        ...
        INSERT INTO inventarios.stock_import_rows(
            company_id, import_id, row_index, sku, barcode, entered_name, product_id, bsale_variant_id, inventory_site_id, inventory_site_location_id, location_id, theoretical_quantity, unit_cost, row_status, entered_site_code, resolved_inventory_site_id, entered_location_code, created_at, created_by
        ) VALUES (
            p_company_id, p_import_id, v_row_index, v_sku, NULLIF(v_barcode, ''), NULLIF(v_entered_name, ''), v_product_id, v_product_bsale_variant_id, v_site_id, v_location_id, NULL, v_quantity, v_cost, CASE WHEN v_has_error THEN 'ERROR' WHEN v_has_warning THEN 'WARNING' ELSE 'VALID' END, NULLIF(v_site_code, ''), v_site_id, NULLIF(v_location_code, ''), v_now, v_actor_id
    ...
    -- Regla exacta aplicada: solo los productos seleccionados de la campana exigen cobertura explicita; ausencia de fila no equivale a cero.
    IF v_campaign_product_scope = 'SELECTED' THEN
        IF v_import_theoretical_scope = 'TOTAL_CAMPAIGN' THEN
            FOR v_issue IN SELECT cp.product_id, cp.sku FROM inventarios.inventory_campaign_products cp WHERE cp.company_id = p_company_id AND cp.campaign_id = v_import_campaign_id AND NOT EXISTS (SELECT 1 FROM inventarios.stock_import_rows r WHERE r.company_id = p_company_id AND r.import_id = p_import_id AND r.row_status <> 'ERROR' AND r.product_id = cp.product_id) LOOP
                v_issue_error_count := v_issue_error_count + 1;
                INSERT INTO inventarios.stock_import_row_issues(company_id, import_id, row_id, row_index, issue_level, issue_code, field, safe_message, metadata)
                VALUES (p_company_id, p_import_id, NULL, NULL, 'ERROR', 'MISSING_SELECTED_PRODUCT_COVERAGE', 'product_id', 'El producto seleccionado no aparece una vez en el archivo.', pg_catalog.jsonb_build_object('product_id', v_issue.product_id, 'sku', v_issue.sku, 'scope', v_import_theoretical_scope));
            END LOOP;
        ELSIF v_import_theoretical_scope = 'BY_SITE' THEN
            FOR v_issue IN SELECT cp.product_id, cp.sku, ics.inventory_site_id, s.code AS site_code FROM inventarios.inventory_campaign_products cp JOIN inventarios.inventory_campaign_sites ics ON ics.company_id = cp.company_id AND ics.campaign_id = cp.campaign_id JOIN inventarios.inventory_sites s ON s.company_id = ics.company_id AND s.id = ics.inventory_site_id WHERE cp.company_id = p_company_id AND cp.campaign_id = v_import_campaign_id AND NOT EXISTS (SELECT 1 FROM inventarios.stock_import_rows r WHERE r.company_id = p_company_id AND r.import_id = p_import_id AND r.row_status <> 'ERROR' AND r.product_id = cp.product_id AND r.resolved_inventory_site_id = ics.inventory_site_id) LOOP
                v_issue_error_count := v_issue_error_count + 1;
                INSERT INTO inventarios.stock_import_row_issues(company_id, import_id, row_id, row_index, issue_level, issue_code, field, safe_message, metadata)
                VALUES (p_company_id, p_import_id, NULL, NULL, 'ERROR', 'MISSING_SELECTED_PRODUCT_COVERAGE', 'resolved_inventory_site_id', 'El producto seleccionado no aparece una vez en cada unidad de la campana.', pg_catalog.jsonb_build_object('product_id', v_issue.product_id, 'sku', v_issue.sku, 'inventory_site_id', v_issue.inventory_site_id, 'site_code', v_issue.site_code, 'scope', v_import_theoretical_scope));
            END LOOP;
        ELSE
            FOR v_issue IN SELECT cp.product_id, cp.sku, ics.inventory_site_id, s.code AS site_code, loc.id AS inventory_site_location_id, loc.code AS location_code FROM inventarios.inventory_campaign_products cp JOIN inventarios.inventory_campaign_sites ics ON ics.company_id = cp.company_id AND ics.campaign_id = cp.campaign_id JOIN inventarios.inventory_sites s ON s.company_id = ics.company_id AND s.id = ics.inventory_site_id JOIN inventarios.inventory_site_locations loc ON loc.company_id = ics.company_id AND loc.inventory_site_id = ics.inventory_site_id LEFT JOIN inventarios.inventory_campaign_site_locations icl ON icl.company_id = ics.company_id AND icl.campaign_site_id = ics.id AND icl.inventory_site_location_id = loc.id WHERE cp.company_id = p_company_id AND cp.campaign_id = v_import_campaign_id AND ((ics.location_scope = 'ALL' AND loc.is_active = true) OR (ics.location_scope = 'SELECTED' AND icl.inventory_site_location_id IS NOT NULL)) AND NOT EXISTS (SELECT 1 FROM inventarios.stock_import_rows r WHERE r.company_id = p_company_id AND r.import_id = p_import_id AND r.row_status <> 'ERROR' AND r.product_id = cp.product_id AND r.resolved_inventory_site_id = ics.inventory_site_id AND r.inventory_site_location_id = loc.id) LOOP
                v_issue_error_count := v_issue_error_count + 1;
                INSERT INTO inventarios.stock_import_row_issues(company_id, import_id, row_id, row_index, issue_level, issue_code, field, safe_message, metadata)
                VALUES (p_company_id, p_import_id, NULL, NULL, 'ERROR', 'MISSING_SELECTED_PRODUCT_COVERAGE', 'inventory_site_location_id', 'El producto seleccionado no aparece una vez en cada ubicacion congelada de la campana.', pg_catalog.jsonb_build_object('product_id', v_issue.product_id, 'sku', v_issue.sku, 'inventory_site_id', v_issue.inventory_site_id, 'site_code', v_issue.site_code, 'inventory_site_location_id', v_issue.inventory_site_location_id, 'location_code', v_issue.location_code, 'scope', v_import_theoretical_scope));
            END LOOP;
        END IF;
    END IF;
