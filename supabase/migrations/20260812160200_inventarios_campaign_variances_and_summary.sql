-- Contratos Backend del Informe Global del Inventario (V1).
--
-- 1. list_inventory_campaign_variances(...)
--    Diferencias globales por producto (teórico oficial de campaña vs contado
--    físico efectivo consolidado de todas las secciones del Inventario).
--
-- 2. get_inventory_campaign_review_summary(...)
--    KPIs del Inventario (stock + operación) para la cabecera del Informe.
--
-- Esquema afectado EXCLUSIVAMENTE: inventarios.
-- Lectura read-only de adquisiciones (products) para resolver catálogo.
