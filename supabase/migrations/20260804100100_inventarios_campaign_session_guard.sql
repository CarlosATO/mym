-- Migration: 20260804100100_inventarios_campaign_session_guard.sql
-- Description: Hotfix 4I.3B.1. Refuerza create_inventory_session_from_campaign_site
--              con dos validaciones defensivas: la unidad debe estar habilitada
--              y una unidad no puede tener mas de una sesion de la misma
--              campana (INV_SESSION_ALREADY_EXISTS). Aditivo: no cambia el
--              contrato de salida ni la rama BSALE_SYNC.
-- Author: Assistant
...
BEGIN
    ...
    SELECT ics.campaign_id, ics.inventory_site_id, ics.location_scope,
           ic.name, ic.product_scope, ic.status, is2.site_type, is2.is_active,
           is2.warehouse_id, is2.name
    INTO v_campaign_id, v_site_id, v_location_scope,
         v_campaign_name, v_product_scope, v_campaign_status,
         v_site_type, v_site_active, v_warehouse_id, v_site_name
    ...
    IF NOT v_site_active THEN
        RAISE EXCEPTION USING ERRCODE='P0001', MESSAGE='INV_INVALID_REQUEST_PAYLOAD',
            DETAIL=pg_catalog.jsonb_build_object('message','La unidad se encuentra deshabilitada.','retryable',false)::text;
    END IF;
    ...
    -- Una unidad no puede crear una segunda sesion de la misma campana.
    -- Se evalua despues del replay para conservar la idempotencia de la clave.
    IF EXISTS (
        SELECT 1 FROM inventarios.sessions s
        WHERE s.company_id = p_company_id
          AND s.campaign_id = v_campaign_id
          AND s.inventory_site_id = v_site_id
    ) THEN
        RAISE EXCEPTION USING ERRCODE='P0001', MESSAGE='INV_SESSION_ALREADY_EXISTS',
            DETAIL=pg_catalog.jsonb_build_object('message','La unidad ya tiene una sesion creada.','retryable',false)::text;
    END IF;
