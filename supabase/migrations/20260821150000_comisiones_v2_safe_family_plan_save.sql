-- COMV2-08C: safe in-place edits and transactional versions for used plans.
...
BEGIN
    ...
    ELSE
        ...
        IF v_issued AND (v_rates_changed OR p_valid_from <> v_plan.valid_from) THEN
            ...
            VALUES (p_company_id, p_supplier_id, btrim(p_plan_code), v_version, 'FAMILY_FIXED_PERCENT', p_valid_from, p_valid_to, 'ACTIVE', true, v_plan.id, 'Nueva versión posterior a liquidación emitida', v_actor, v_actor)
            RETURNING * INTO v_plan;
            v_target_plan_id := v_plan.id;
        ELSE
