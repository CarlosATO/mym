-- Resolve sales by canonical variant id first, then by an unambiguous code.
-- The RPC intentionally contains aggregate metrics only, not per-sale dates.

ALTER TABLE integraciones.bsale_stock_break_summary_60d
    ADD COLUMN IF NOT EXISTS sales_identity_resolved_60d boolean NOT NULL DEFAULT false,
    ADD COLUMN IF NOT EXISTS sales_identity_method_60d text NOT NULL DEFAULT 'NONE';

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'integraciones.bsale_stock_break_summary_60d'::regclass
          AND conname = 'bsale_stock_summary_sales_identity_method_check'
    ) THEN
        ALTER TABLE integraciones.bsale_stock_break_summary_60d
            ADD CONSTRAINT bsale_stock_summary_sales_identity_method_check
            CHECK (sales_identity_method_60d IN ('VARIANT_ID', 'VARIANT_CODE', 'NONE', 'AMBIGUOUS'));
    END IF;
END;
$$;

ALTER FUNCTION integraciones.refresh_bsale_stock_break_read_models(uuid, int, date, date)
    RENAME TO refresh_bsale_stock_break_read_models_before_sales_identity;
...
BEGIN
    ...
    WITH summary_rows AS (
        ...
        FROM classified c
        JOIN sales_daily s
          ON s.company_id = c.company_id AND s.code = c.effective_code
    ...
    SET sales_identity_resolved_60d = (c.identity_method IN ('VARIANT_ID', 'VARIANT_CODE')),
        ...
        units_sold_with_stock_60d = CASE WHEN c.identity_method IN ('VARIANT_ID', 'VARIANT_CODE')
            THEN COALESCE(m.units_with_stock, 0) ELSE 0 END,
        units_sold_unknown_days_60d = CASE WHEN c.identity_method IN ('VARIANT_ID', 'VARIANT_CODE')
            THEN COALESCE(m.units_unknown, 0) ELSE 0 END,
        sales_rate_with_stock_60d = CASE
            WHEN c.identity_method IN ('VARIANT_ID', 'VARIANT_CODE') AND summary.days_with_stock_60d > 0
            THEN COALESCE(m.units_with_stock, 0) / summary.days_with_stock_60d
            ELSE NULL
        END,
        positive_sales_without_stock_60d = CASE WHEN c.identity_method IN ('VARIANT_ID', 'VARIANT_CODE')
            THEN COALESCE(m.positive_without_stock, 0) ELSE 0 END,
        refreshed_at = now()
    FROM classified c
    LEFT JOIN metrics m ON m.variant_id = c.variant_id
...
AS $$
    SELECT COALESCE(jsonb_object_agg(summary.variant_id::text, jsonb_build_object(
        ...
        'salesIdentityResolved', summary.sales_identity_resolved_60d,
        'salesIdentityMethod', summary.sales_identity_method_60d,
        'unitsSoldWithStock', CASE WHEN summary.sales_identity_resolved_60d THEN summary.units_sold_with_stock_60d ELSE NULL END,
        'unitsSoldUnknownDays', CASE WHEN summary.sales_identity_resolved_60d THEN summary.units_sold_unknown_days_60d ELSE NULL END,
        'salesRateWithStock', CASE WHEN summary.sales_identity_resolved_60d THEN summary.sales_rate_with_stock_60d ELSE NULL END,
        'knownDays', summary.known_days_60d,
        'evidenceCoveragePct', summary.evidence_coverage_pct,
        'positiveSalesWithoutStock', CASE WHEN summary.sales_identity_resolved_60d THEN summary.positive_sales_without_stock_60d ELSE NULL END,
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE i.company_id = summary.company_id
          AND i.office_id = summary.office_id
