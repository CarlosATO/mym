-- Worker account: detailed read model V2. Read-only contracts only.
...
BEGIN
  ...
  WITH charge_balances AS (
    ...
    SELECT employee_id,
      coalesce(sum(greatest(original_amount - approved_paid_amount, 0)), 0)::numeric AS official_balance,
      coalesce(sum(greatest(original_amount - approved_paid_amount, 0)) FILTER (WHERE source_type = 'MERMA'), 0)::numeric AS merma_balance,
      coalesce(sum(greatest(original_amount - approved_paid_amount, 0)) FILTER (WHERE source_type = 'BSALE_BOLETA'), 0)::numeric AS bsale_boleta_balance,
      count(*) AS active_charge_count,
      count(*) FILTER (WHERE greatest(original_amount - approved_paid_amount, 0) > 0) AS open_charge_count,
      max(charge_at) AS last_charge_at
    FROM charge_balances
    GROUP BY employee_id
  ), account_payments AS (
    SELECT employee_id,
      coalesce(sum(amount) FILTER (WHERE status = 'APPROVED'), 0)::numeric AS approved_payments,
    ...
    FROM mermas.worker_payments
    WHERE company_id = p_company_id
    GROUP BY employee_id
  ), account_employees AS (
    SELECT employee_id FROM account_charges
    UNION
    SELECT employee_id FROM account_payments
  )
  SELECT e.id,
    ...
    coalesce(c.merma_balance, 0)::numeric,
    coalesce(c.bsale_boleta_balance, 0)::numeric,
    coalesce(c.active_charge_count, 0), coalesce(c.open_charge_count, 0),
    c.last_charge_at, p.last_payment_at
  FROM account_employees a
  JOIN rrhh.employees e ON e.id = a.employee_id
  LEFT JOIN account_charges c ON c.employee_id = e.id
  LEFT JOIN account_payments p ON p.employee_id = e.id
  WHERE v_search = ''
  ...
  ORDER BY greatest(coalesce(c.official_balance, 0), 0) DESC,
...
GRANT EXECUTE ON FUNCTION mermas.get_worker_accounts_v2(uuid, uuid, text),
  mermas.get_worker_account_detail_v2(uuid, uuid, uuid) TO service_role;
