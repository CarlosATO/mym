...
async function main() {
  ...
  const { data: health } = await iDb.from('vw_bsale_mirror_sync_health').select('*');
  console.log(health);
  ...
  expectedFolios.forEach(folio => {
    const d = blockDetails.find(x => x.document_number === folio);
    if (d) {
      const coincide = d.logistic_net_quantity === d.raw_quantity ? 'SÍ' : 'NO';
      console.log(`| ${folio} | SÍ | SÍ | ${d.raw_quantity} | ${d.logistic_net_quantity} | ${d.document_type_id} | ${d.emission_date} | - | ${coincide} |`);
    } else {
